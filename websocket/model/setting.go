...
// Setting 设置表
type Setting struct {
	Key         string `gorm:"column:name;type:varchar(30);comment:设置项标示;NOT NULL" json:"name"`
	Description string `gorm:"column:description;type:varchar(255);comment:设置项描述;NOT NULL" json:"description"`
	Values      string `gorm:"column:value;type:mediumtext;comment:设置内容（json格式）;NOT NULL" json:"value"`
