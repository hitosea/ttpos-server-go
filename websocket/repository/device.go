...
func (r *DeviceRepository) GetBindCount(companyId uint64, source string) uint {
	var c int64
	r.dbm.GetDB(companyId).Model(&model.Device{}).Where("source = ? AND finally_login_id > 0", source).Count(&c)
	return uint(c)
}

func (r *DeviceRepository) GetRecordBySourceAndDeviceId(companyId uint64, source string, deviceId string) model.Device {
	var Device model.Device
	r.dbm.GetDB(companyId).Model(&model.Device{}).Where("source = ? AND device_id = ?", source, deviceId).First(&Device)
	return Device
}

func (r *DeviceRepository) Update(companyId uint64, id uint, vars map[string]interface{}) error {
	return r.dbm.GetDB(companyId).Model(&model.Device{}).Where("id = ?", id).Updates(vars).Error
}

func (r *DeviceRepository) Create(companyId uint64, Device model.Device) error {
