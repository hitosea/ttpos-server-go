...
class UserShiftLog extends Controller
{
    ...
    public function detail()
    {
        ...
        // 有快照就返回快照数据
        if ($detail->snapshot && $detail->snapshot->content) {
            $detail = json_decode($detail->snapshot->content, true);
            $salesInfo = $detail['salesInfo'];
            foreach ($salesInfo as &$item) {
                $name = json_decode($item['name'], true) ?? [];
                $item['name_text'] = $name[request()->header("language")] ?? $item['name_text'];
            }
            //
            $incomes = [];
            foreach ($detail['incomes'] as $income) {
                if ($income['pay_type'] == -1) {
                    $income['pay_type_name'] = __(OrderPayTypeEnum::data()[$income['pay_type']]['name'], request()->header("language"));
                } else {
                    $income['pay_type_name'] = __($income['pay_type_name'], request()->header("language"));
                }
                $incomes[] = $income;
            }
            $detail['incomes'] = $incomes;
