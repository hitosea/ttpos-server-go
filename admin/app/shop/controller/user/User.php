...
class User extends Controller
{
     ...
     * @Apidoc\Param("user_id", type="int", require=true, default="", desc="用户id")
     * @Apidoc\Param("source", type="int", require=true, default="", desc="充值方式 0-余额 1-积分 2-余额+积分(v1.0.8)")
     ...
     *     @Apidoc\Param("gift_balance", type="float", require=true, default="", desc="赠送金额（v1.0.8）"),
     ...
     *    @Apidoc\Param("mode", type="string", require=true, default="", desc="充值方式 inc-增加 dec-减少 final-最终金额"),
     *    @Apidoc\Param("value", type="float", require=true, default="", desc="充值金额"),
     *    @Apidoc\Param("remark", type="string", require=true, default="", desc="备注"),
     * }),
     * })
     * @Apidoc\Returned()
     ...
     * @Apidoc\Returned()
     */
    public function deduct($user_id)
    {
        $data = $this->postData();
        ...
        if ($model->deduct($user_name, $source, $data)) {
            return $this->renderSuccess('操作成功');
        }
        return $this->renderError($model->getError() ?: '操作失败');
    }
