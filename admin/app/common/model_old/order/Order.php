...
class Order extends BaseModelOrder
{
    ...
    public function reloadPrice($order_id, $sub_order = null, $setting_param = [])
    {
        ...
        //
        $tableId = $order['parent_id'] == 0 ? $order['table_id'] : (self::where('order_id', $order['parent_id'])->value('table_id') ?: 0); // 主单/子单table_id
        //
        $pay_money = 0;
        $order_price = 0;
        $points_bonus = 0;
        ...
        $setting_service_money = (
            $serviceFee['is_open'] && 
            $serviceFee['charge_type'] == 1 && 
            self::checkOrderNeedServiceCharge($tableId, $serviceFee)
        ) ? $serviceFee['service_charge'] : 0;
        //
        $productUpdateArr = [];
        ...
        foreach ($order_product as $product) {
            ...
            if ($serviceFee['is_open'] && $serviceFee['charge_type'] == 2 && self::checkOrderNeedServiceCharge($tableId, $serviceFee)) {
                // 折扣后的商品服务费
    ...
    public function getBackPayPrice($product_source = self::CASHIER_PRODUCT_SOURCE)
    {
        ...
        if ($this['discount_change_price'] > 0 || $this['discount_change_price'] == -1) {
            ...
            // table_id
            $tableId = $order['parent_id'] == 0 ? $order['table_id'] : (self::where('order_id', $order['parent_id'])->value('table_id') ?: 0);
            if ($serviceFee['is_open'] && self::checkOrderNeedServiceCharge($tableId, $serviceFee)) {
     ...
     * 检查订单是否需要收取服务费
     */
    public static function checkOrderNeedServiceCharge($tableId, $serviceFee)
    {
        // 全部应用
        if ($serviceFee['apply_scope'] == '1') {
            return true;
        }
        // 部分应用
        if ($serviceFee['apply_scope'] == '2') {
            // 适用范围-点餐
            if ($serviceFee['apply_scope_ordering'] == '1' && $tableId == 0) {
                return true;
            }
            // 适用范围-桌台
            if ($serviceFee['apply_scope_table'] == '1' && 
                $tableId > 0 && 
                in_array($tableId, $serviceFee['apply_scope_table_list'])
            ) {
                return true;
            }
        }
