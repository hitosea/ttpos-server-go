...
class Base
{
     ...
     * 保存二维码码到文件
     */
    protected function saveMpQrcodeToDir($path, $savePath)
    {
        $qrcode = new QrCode(base_url() . $path);
        // 文件名称
        $fileName = 'qrcode_' . md5($path) . '.png';
        // 保存二维码到文件
     ...
     * 保存小程序码到文件
     */
    protected function saveGroupQrcode($app_id, $scene, $page)
    {
        // 文件目录
        $dirPath = root_path('public') . "/temp/{$app_id}/image_wx";
        !is_dir($dirPath) && mkdir($dirPath, 0755, true);
     ...
     * 保存普通二维码到文件
     */
    protected function saveGroupMpQrcode(\Endroid\QrCode\QrCode $qrcode, $app_id, $scene, $source)
    {
        // 文件目录
        $dirPath = root_path('public') . "/temp/{$app_id}/{$source}";
        !is_dir($dirPath) && mkdir($dirPath, 0755, true);
