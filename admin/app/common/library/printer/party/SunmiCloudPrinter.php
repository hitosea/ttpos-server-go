...
class SunmiCloudPrinter
{
    ...
    //
    private $APP_ID = "00000000000000000000000000000000";
    private $APP_KEY = "00000000000000000000000000000000";

    public function __construct($dots_per_line = 384, $app_id = '', $app_key = '')
    {
        $this->APP_ID = $app_id;
    ...
    {
        $msg = $body_data . $this->APP_ID . $timestamp . $nonce;
        return hash_hmac("sha256", $msg, $this->APP_KEY, false);
    }
    ...
    // [GS !] 设置字符大小。
    function setCharacterSize($h, $w, $is = true)
    {
        $n = 0;
        if ($h >= 1 && $h <= 8)
            $n |= ($h - 1);
    ...
    // [GS V m]切纸。
    function cutPaper(bool $reminderSound = true)
    {
        // 添加提示音
        if ($reminderSound) {
            $this->appendText("\x1B\x42\x03\x02");
        }
        // 
        $this->orderData .= sprintf("1d56%02x", 0);
    }
    ...
    function appendImage($image_file, $mode, $max_width = 0)
    {
        ...
        for ($y = 0; $y < $h; $y++) {
            for ($x = 0; $x < $w; $x++) {
                ...
                $g = ($rgb >> 8) & 0xff;
                $b = ($rgb) & 0xff;
                $grayscale[$i++] = (int)(($r * 11 + $g * 16 + $b * 5) / 32) & 0xff;
            }
        }
        imagedestroy($image);
    ...
    // x, y: origin of the print area[ESC W] Set print area in page mode.
