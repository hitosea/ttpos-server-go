...
class helper
{
    ...
    public static function amountPermillage($amount)
    {
        if (!$amount) {
            return 0;
        }
        $formatted_amount = number_format($amount, 2, '.', ',');
        $formatted_amount = rtrim($formatted_amount, '0');          // 去除小数部分后面为0的情况
        if (substr($formatted_amount, -1) == '.') {                 // 如果最后一个字符是小数点，则也一并去除
            $formatted_amount = rtrim($formatted_amount, '.');
        }
        return $formatted_amount; // 输出 1,234,567.89
    }
    ...
    public static function getArrayColumnNumSum($array, $column, $num_column)
    {
        $sum = 0; //
        foreach ($array as $item) {
            $sum += $item[$column] * $item[$num_column] * 100;
        }
        return $sum / 100;
    }
