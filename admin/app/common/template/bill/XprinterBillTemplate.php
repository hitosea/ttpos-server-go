 ...
 * 心烨 结账单模版
 */
class XprinterBillTemplate extends BaseTemplate
{
    /**
     * 生成模版
    ...
    public function create(OrderModel $order, $printerType, $isPrePrint = true)
    {
        ...
        // 
        $isXinyeWifi = $printerType == PrinterTypeEnum::XPRINTER_WIFI || $printerType == PrinterTypeEnum::CODESOFT_LAN;
        $isCodesoftLan = $printerType == PrinterTypeEnum::CODESOFT_LAN;
        ...
        */
        if ($template == 1) {
            $printer->setAlignment(SunmiCloudPrinter::ALIGN_LEFT);
            $printer->setLineSpacing($isXinyeWifi ? 90 : 50);
            $printer->setCharacterSize(1, 1);
            $printer->appendText("\x1D\x21\x01");
            $printer->setPrintModes(true, true, false);
            if ($order['table_no']) {
                $printer->setLineSpacing($isXinyeWifi ? 120 : 50);
        ...
        */ 
            ...
            //
            $printer->lineFeed();
            $printer->setLineSpacing($printerType ==  PrinterTypeEnum::XPRINTER_WIFI ? 90 : 50);
            $printer->setCharacterSize(1, 1);
            $printer->appendText("\x1D\x21\x01");
            $printer->setPrintModes(true, true, false);
            if ($order['table_no']) {
        ...
        */ 
            ...
            if ($company) {
                ...
                $printer->setLineSpacing($isXinyeWifi ? 40 : 20);
                $printer->lineFeed(2);
            }
            if ($chainNumber) {
                if ($isXinyeWifi) {
            ...
            $printer->setPrintModes(false, false, false);
            $printer->setAlignment(SunmiCloudPrinter::ALIGN_LEFT);
            $printer->appendText(__("收银员") . ': ' . $order->cashier?->real_name ?: $order->cashier?->user_name ?: '');
            $printer->lineFeed();
            if ($payTime) {
        ...
        foreach ($products as $item) {
            ...
            foreach ($item['buffetCustomerType'] ?? [] as $buffet) {
                ...
                $printer->appendText(printText($buffetNameText, helper::amountPermillage($buffet['price']) . '*' . $buffet['num'], $this->getPriceAndUnit($buffet['total_price']), $width, $leftWidth, $centerWidth, $rightWidth));
                $printer->lineFeed();
                $printer->setLineSpacing(50);
                $printer->lineFeed();
                $printer->setLineSpacing(60);
            ...
            foreach ($item['buffetDiscount'] ?? [] as $discount) {
                ...
                $printer->appendText(printText($discount['name_text'], helper::amountPermillage($discount['price']) . '*' . $discount['num'], '-' . $this->getPriceAndUnit($discount['total_price']), $width, $leftWidth, $centerWidth, $rightWidth));
                $printer->lineFeed();
                $printer->setLineSpacing(50);
                $printer->lineFeed();
                $printer->setLineSpacing(60);
            ...
            foreach ($item['delay'] ?? [] as $delay) {
                ...
                $printer->appendText(printText($delay['name_text'], helper::amountPermillage($delay['price']) . '*' . $delay['num'], $this->getPriceAndUnit($delay['total_price']), $width, $leftWidth, $centerWidth, $rightWidth));
                $printer->lineFeed();
                $printer->setLineSpacing(50);
                $printer->lineFeed();
                $printer->setLineSpacing(90);
            ...
            foreach ($item['products'] as $product) {
                ...
                $printer->appendText(printText($productName, helper::amountPermillage($product['product_price']) . '*' . $product['total_num'], $this->getPriceAndUnit($product_total_product_price), $width, $leftWidth, $centerWidth, $rightWidth));
                $printer->lineFeed();
                $printer->setLineSpacing(50);
                $printer->lineFeed();
                $printer->setLineSpacing(60);
        ...
        if ($template == 3) {
            $printer->appendText(printText(__("商品数量") . ': ' . $productNum, '', __("商品金额") . ': ' . $this->getPriceAndUnit($total_product_price), $width));
            $printer->lineFeed();
        } else {
        ...
        $printer->setPrintModes(true, true, false);
        $printer->appendText(printText(__("合计应收"), '', $this->getPriceAndUnit($order['actual_receive_price']), $width, 34));
        ...
        if ($order['consumption_tax_money'] > 0 && $order['consumption_tax_type'] == 1 && ($this->consumptionTax == 1 || $this->consumptionTax == 2)) {
            $printer->lineFeed();
            $printer->appendText("------------------------------------------------\n");
            $printer->setLineSpacing($isXinyeWifi ? 20 : 5);
            $printer->lineFeed();
            $printer->setAlignment(SunmiCloudPrinter::ALIGN_RIGHT);
        ...
        if ($order->pay_status['value'] == OrderPayStatusEnum::SUCCESS) {
            ...
            if (count($payTypes) > 0) {
                $printer->lineFeed();
                $printer->setLineSpacing($isCodesoftLan ? 80 : 90);
                $printer->appendText("------------------------------------------------\n");
                foreach ($payTypes as $payType) {
                    $printer->appendText(printText(__("支付方式"), '', $payType['pay_type']['text'], $width, 20, 0, 28));
                    $printer->lineFeed();
                    $printer->appendText(printText(__("实收金额"), '', $payType['value'] == -1 ? '0' : $this->getPriceAndUnit($payType['price']), $width, 34));
        ...
        if ($order->user) {
            $printer->lineFeed();
            $printer->setLineSpacing($isCodesoftLan ? 45 : 90);
            $printer->appendText("------------------------------------------------\n");
            //
            $pointnum = $order['is_free'] != 0 ? 0 : ($order->points_bonus ?: 0);
            $balance = $payTime ? ($order['surplus_balance'] ?? 0) : $order->surplusBalance();
            $printer->appendText(printText(__("会员剩余余额"), '',  $this->getPriceAndUnit($balance), $width, 34) . "\n");
