 ...
 * 心烨 交班单模版
 */
class XprinterHandoverTemplate extends BaseTemplate
{
    /**
     * 生成模版
     */
    public function create($printerType, $data, $shopName, $isPrePrint)
    {
        ...
        */
        if ($template == 2 || $template == 3) {
            ...
            // 异常信息
            if ($template == 3) {
                $printer->appendText(printText(__("退菜次数"), '', helper::amountPermillage($abnormalData['refund_product_times'] ?? 0) . '', $width));
                $printer->lineFeed(1);
                $printer->appendText(printText(__("退款次数"), '', helper::amountPermillage($abnormalData['refund_times'] ?? 0) . '', $width));
                $printer->lineFeed(1);
                $printer->appendText(printText(__("反结账次数"), '', helper::amountPermillage($abnormalData['reverse_settle_times'] ?? 0) . '', $width));
                $printer->lineFeed(1);
                $printer->appendText(printText(__("赠菜次数"), '', helper::amountPermillage($abnormalData['product_free_times'] ?? 0) . '', $width));
                $printer->lineFeed(1);
                $printer->appendText(printText(__("免单次数"), '', helper::amountPermillage($abnormalData['free_order_times'] ?? 0) . '', $width));
                $printer->lineFeed(1);
                $printer->appendText(printText(__("转菜次数"), '', helper::amountPermillage($abnormalData['product_move_times'] ?? 0) . '', $width));
                $printer->lineFeed(1);
                $printer->appendText(printText(__("单品改价次数"), '', helper::amountPermillage($abnormalData['change_price_times'] ?? 0) . '', $width));
                $printer->lineFeed(1);
                $printer->appendText(printText(__("整单改价次数"), '', helper::amountPermillage($abnormalData['change_order_price_times'] ?? 0) . '', $width));
                $printer->lineFeed(1);
                $printer->appendText(printText(__("整单折扣次数"), '', helper::amountPermillage($abnormalData['discount_order_times'] ?? 0) . '', $width));
                $printer->lineFeed(1);
                $printer->appendText(printText(__("整单抹零次数"), '', helper::amountPermillage($abnormalData['round_order_times'] ?? 0) . '', $width));
                $printer->lineFeed(1);
                $printer->appendText("------------------------------------------------");
            }
            // 会员充值
            ...
            $printer->appendText("------------------------------------------------");
            $printer->setPrintModes(true, false, false);
            $printer->appendText(printText(__('高峰时间'), __('订单数'), $this->lang == 'en' ? 'Amount' : __('订单金额'), $width, 24 - (($this->lang == 'en' || $this->lang == 'th' || $this->lang == 'tr') ? 4 : 0), 20, 16));
            $printer->setPrintModes(false, false, false);
            $printer->lineFeed();
