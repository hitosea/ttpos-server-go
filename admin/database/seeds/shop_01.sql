...
CREATE TABLE IF NOT EXISTS `ttpos_sale_bill` (
    ...
    -- 随订单修改而更新的字段
    `amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '订单金额(折后价),关联销售订单的总金额之和',
    `origin_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '订单金额(折前价)。商品未含税时，订单金额(折前价)=商品金额+服务费+税费。商品已含税时，订单金额(折前价)=商品金额（含商品消费税）+服务费+税费（只有服务费税）',
    

    -- 完成账单才记录的字段
    `product_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '商品金额,关联销售订单的商品金额之和',
    `service_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '服务费,关联销售订单的服务费之和',
    `tax_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '税费,关联销售订单的税费之和',
    `custom_discount_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '自定义折扣费用,关联销售订单的会员折扣费用之和',
    `member_discount_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '会员折扣费用,关联销售订单的会员折扣费用之和',
    `gift_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '赠菜金额,关联销售订单的赠菜金额之和',
    `free_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '免单金额,关联销售订单的免单金额之和',

    `payment_commission_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '支付手续费,多次支付的支付手续费之和',
    `payment_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '支付金额,支付金额-订单总金额=支付手续费',
    `product_original_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '原始商品金额。 商品原始金额=(订单.原始商品金额)之和。',
...
CREATE TABLE IF NOT EXISTS `ttpos_sale_order` (
    ...
    -- 订单数据变动时要重新计算的字段
    `member_discount_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '总会员折扣金额。总会员折扣金额=(订单商品.会员折扣金额)之和',
    `custom_discount_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '总自定义折扣金额。总自定义折扣金额=(订单商品.自定义折扣金额)之和',
    `zero_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '优惠折扣抹零金额。',
    `product_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '商品金额，订单商品的最终单价(折后价)之和。商品已含税时，该金额包括了税费。当商品未含税时，该金额不包括税费',
    `product_original_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '原始商品金额(折前价)。 商品原始金额=订单商品的销售价(折前价)之和。',
    `service_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '服务费固定服务费时，服务费=固定服务费；按比例收服务费时，服务费=(订单商品.总服务费)之和',
    `tax_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '税费。税费=(订单商品.总税费)之和',
    `amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '应收金额。商品未含税时，总金额=商品金额+服务费+税费。商品已含税时，总金额=商品金额（含商品消费税）+服务费+税费（只有服务费税）',
    `origin_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '原始应收金额。原始应收金额=商品金额+服务费+消费税。商品未含税时，原始应收金额=商品金额+服务费+消费税（商品消费税税费+服务费税费）。商品已含税时，原始应收金额=商品金额（包含商品消费税税费）+服务费+服务费税费。',
    -- 免单。
    `is_free` INT(10) NOT NULL DEFAULT 0 COMMENT '是否免单, 0-否 1-是',
    `free_reason` TEXT COMMENT '免单原因',
    -- 订单设置相关
    `member_discount_rate`  DECIMAL(20, 4) NOT NULL DEFAULT 1.00 COMMENT '会员折扣率(0-100%)，默认100%，取值范围0-1，如折扣率为10%，则取值为0.1',
    `member_card_discount_rate`  DECIMAL(20, 4) NOT NULL DEFAULT 1.00 COMMENT '会员卡折扣率(0-100%)，默认100%，取值范围0-1，如折扣率为10%，则取值为0.1',
    `custom_discount_rate`  DECIMAL(20, 4) NOT NULL DEFAULT 1.00 COMMENT '自定义折扣率(0-100%)，默认100%，取值范围0-1，如折扣率为10%，则取值为0.1',
    `custom_amount`  DECIMAL(20, 4) NOT NULL DEFAULT -1 COMMENT '整单改价金额。改价后，应收金额=整单改价金额，前端优先显示改价后的金额，改价金额不能为负数。当为-1时，表示不改价，显示amount改收金额',
    ...
    -- 积分抵扣相关
    `pay_points`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '抵扣积分,用了多少积分进行抵扣',
    `pay_points_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '抵扣金额,积分 抵扣了多少金额',
    `points_exchange_rate` DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '积分抵扣汇率,1积分抵扣多少元',
    `auto_points_exchange` INT(10) NOT NULL DEFAULT 0 COMMENT '积分抵扣类型,0-手动抵扣 1-自动抵扣',
    -- 结账完成后才记录的字段
    `coupon_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '优惠券抵扣金额,抵扣了多少金额',
    `payment_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '已支付金额,关联付款单的支付金额之和。',
    `change_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '找零金额,结账完成后才记录',
    `zero_checkout_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '结账抹零金额。',
    `final_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '最终应收金额。最终应收金额=应收金额+手续费-结账抹零金额',
    `payment_commission_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '支付手续费,关联付款单的支付手续费之和',
    `gift_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '赠菜金额,(销售订单赠菜商品.总最终单价)之和',
    `gift_points`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '赠送积分. 赠送积分=应收金额amount*积分赠送比例.',
    `gift_points_rate` DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '赠送积分比例. 取值范围0-1。结账后记录，不受后台改变',
    `gift_points_type` INT(10) NOT NULL DEFAULT 0 COMMENT '赠送积分类型, 0-按比例赠送 1-按人数固定金额赠送',
    `member_balance`   DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '会员余额.会员消费本单后剩余的余额',
...
CREATE TABLE IF NOT EXISTS `ttpos_sale_order_coupon` (
    ...
    `coupon_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '优惠券抵扣金额，实际抵扣金额',
    `coupon_origin_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '优惠券原始金额(面值)',
...
CREATE TABLE IF NOT EXISTS `ttpos_member_sale_order` (
    ...
    `product_num`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '商品数量.订单中商品的总数量，商品A数量2，商品B数量1，则商品数量为3',
    `product_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '商品金额,折前价，已含税',
    `origin_product_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '商品原价,折前价，已含税',
    `member_discount_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '会员折扣',
    `amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '订单总金额.商品金额-会员折扣+配送费',
    ...
    `delivery_distance`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '配送距离，单位km',
    `delivery_fee_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '配送费',
    `delivery_fee_distance`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '距离费送费',
    `delivery_fee_min_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '起步配送费',
    `delivery_fee_base_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '基础配送费',
    `delivery_fee_per_km`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '每公里配送费',
    ...
    `remaining_distance`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '剩余距离',
    -- 排序相关
...
CREATE TABLE IF NOT EXISTS `ttpos_sale_bill_setting` (
    ...
    `service_fee_value` DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '服务费值,服务费类型为1时,服务费值为固定金额,服务费类型为2和3时,服务费值为%比例',
...
CREATE TABLE IF NOT EXISTS `ttpos_payment_order` (
    ...
    `payment_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '支付金额',
    `payment_commission_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '支付手续费,支付金额*支付手续费百分比',
    `amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '实收金额，实收金额=支付金额+支付手续费',
    ...
    -- 余额支付相关，用于反结账时退款
    `balance_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '主账户金额,用于反结账时退款',
    `gift_balance_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '赠送帐户金额,用于反结账时退款',
    -- 时间
    `create_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建时间(时间戳)',
    `update_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '更新时间(时间戳)',
    `delete_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '删除时间(时间戳)',
...
CREATE TABLE `ttpos_ll_payment_order` (
    ...
    `order_amount`  DECIMAL(20, 4) DEFAULT 0.00 COMMENT 'lianlian订单金额',
    `order_currency` varchar(50) DEFAULT '' COMMENT 'lianlian订单货币',
    `commission_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '支付手续费,支付金额*支付手续费百分比',
...
CREATE TABLE IF NOT EXISTS `ttpos_payment_method` (
    ...
    `fee_percent`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '手续费百分比,取值范围0-1',
...
CREATE TABLE IF NOT EXISTS `ttpos_sale_order_product` (
    ...
    `device_id` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '设备ID,用于标识订单来源设备.来源h5时，device_id为h5',
    -- 价格信息
    `flavor_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '规格原价（单商品）,仅某规格商品的原价',
    `sauce_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '小料价（单商品）,所有小料的价格之和',
    `product_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '原始单价（单商品）,规格原价+小料价',
    ...
    -- 总销售价=销售价*数量
    `sale_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '销售价（单商品，折前价）,当自定义价格时，销售价=自定义价格,否则销售价=原始单价',
    `sale_price_no_tax`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '销售价,未含税价格（折前）',
    -- 税率
    `tax_rate`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '税率,单位%.加购时记录税率,结账时再重新核算',
    -- 折扣率=会员折扣率*会员卡折扣率*自定义折扣率
    `member_discount_rate` DECIMAL(20, 4) NOT NULL DEFAULT 1 COMMENT '会员折扣率(0-100%)',
    `member_card_discount_rate` DECIMAL(20, 4) NOT NULL DEFAULT 1 COMMENT '会员卡折扣率(0-100%)',
    `custom_discount_rate` DECIMAL(20, 4) NOT NULL DEFAULT 1 COMMENT '自定义折扣率(0-100%)',
    `open_overall_discount` INT(10) NOT NULL DEFAULT 1 COMMENT '是否开启 Overall 折扣, 0-否 1-是',
    -- 会员折扣后的价格
    `member_discount_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '会员折扣后的价格（单商品）=销售价*会员折扣率*会员卡折扣率',
    -- 最终单价=销售价*折扣率；总最终单价=最终单价*商品数量
    `price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '最终单价(单商品，会员、会员卡和优惠折扣后，折后价)。销售价*折扣率',
    -- 单个商品总税费=商品税费+服务费税费；总税费=单个商品总税费*商品数量
    `service_tax_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '服务费税费（单商品）,0-不收取税费；收取时，服务费税费=服务费*税率',
    `tax_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '商品税费（单商品）。商品已含税时，税费=规格原价*(1-1/(1+税率))；商品未含税时，税费=原始单价*税率',
    -- 服务费; 总服务费=单个商品服务费*商品数量
    `service_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '服务费（单商品）,0-固定服务费 大于0-按比例收服务费；商品已含税时，服务费=(最终单价-商品税费)*服务费比例；商品未含税时，服务费=最终单价*服务费比例',
    -- 单个商品应收金额=最终单价+服务费+总税费; 总应收金额=单个商品应收金额*商品数量
    `total_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '应收金额(单商品)。商品已含税时，应收金额(单商品)=(最终单价-商品税费)+服务费+总税费；商品未含税时，应收金额(单商品)=最终单价+服务费+总税费',
    `origin_total_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '应收金额(单商品)。商品已含税时，应收金额(单商品)=(销售价-商品税费)+服务费+总税费；商品未含税时，应收金额(单商品)=销售价+服务费+总税费',
    -- 打折金额；总打折金额=单个商品打折金额*商品数量
    `discount_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '打折金额（单商品）=销售价-最终单价。校验：打折金额=会员折扣金额+自定义折扣金额',
    -- 会员折扣金额
    `member_discount_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '会员折扣金额（单商品）=销售价*（1-会员折扣率*会员卡折扣率）',
    -- 自定义折扣金额
    `custom_discount_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '自定义折扣金额（单商品）。自定义折扣金额（单商品）=会员折扣后的价格（单商品）*(1-自定义折扣率) 。校验：自定义折扣金额（单商品）=销售价 - 最终单价（单商品）-会员折扣金额（单商品）；注意，不能这样算，自定义折扣金额（单商品）=销售价*(1-自定义折扣率)',
    -- 状态值
...
CREATE TABLE IF NOT EXISTS `ttpos_sale_order_peak_time` (
  ...
  `amount`  DECIMAL(20, 4) DEFAULT 0.00 COMMENT '订单金额',
  `cashier_uuid` BIGINT NOT NULL DEFAULT 0 COMMENT '收银员ID',
...
CREATE TABLE IF NOT EXISTS `ttpos_h5_order` (
    ...
    `member_discount_rate`  DECIMAL(20, 4) NOT NULL DEFAULT 1 COMMENT '会员折扣率(0-100%).接单和拒单后从sale_order_product表获取，不再改变',
    `member_card_discount_rate`  DECIMAL(20, 4) NOT NULL DEFAULT 1 COMMENT '会员卡折扣率(0-100%).接单和拒单后从sale_order_product表获取，不再改变',
    `custom_discount_rate`  DECIMAL(20, 4) NOT NULL DEFAULT 1 COMMENT '自定义折扣率(0-100%).接单和拒单后从sale_order_product表获取，不再改变',
    -- end 记录信息，用于财务核算或门店营业管理
    `product_total_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '商品总价。接单和拒单后从sale_order_product表获取，不再改变',
    `total_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '订单金额. 订单金额=商品总价*折扣率。接单和拒单后从sale_order_product表获取，不再改变',
...
CREATE TABLE IF NOT EXISTS `ttpos_h5_order_product` (
    ...
    `price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '最终单价（折后价）。接单和拒单后从sale_order_product表获取，不再改变',
    `sale_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '销售价（折前价）。接单和拒单后从sale_order_product表获取，不再改变',
    `num`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '最终商品数量.接单和拒单后从sale_order_product表获取，不再改变',
...
CREATE TABLE IF NOT EXISTS `ttpos_sale_order_product_bom` (
    ...
    `price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '单价,不随后台更新，记录加购时的价格。结账时要校验价格是否变动',
...
CREATE TABLE IF NOT EXISTS `ttpos_sale_order_discount_strategy` (
    ...
    `value`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '优惠策略值',
...
CREATE TABLE IF NOT EXISTS `ttpos_production_order_product` (
    ...
    `num`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '商品数量',
...
CREATE TABLE IF NOT EXISTS `ttpos_buffet_package` (
    ...
    `multi_language_name_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '多语言名称ID',
    `actual_sale_num`  DECIMAL(20, 4) NOT NULL DEFAULT 0.0000 COMMENT '实际销量。每次卖出时,实际销量增加',
...
CREATE TABLE IF NOT EXISTS `ttpos_buffet_delay` (
    ...
    `price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '价格',
    `status` INT(10) NOT NULL DEFAULT 1 COMMENT '状态 0-禁用 1-启用',
    `create_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建时间(时间戳)',
    `update_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '更新时间(时间戳)',
    `delete_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '删除时间(时间戳)',
...
CREATE TABLE IF NOT EXISTS `ttpos_sale_order_buffet_delay_product` (
    ...
    `price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '价格,下单时固定不受后台改变，结账时再检查是否改变',
...
CREATE TABLE IF NOT EXISTS `ttpos_sale_order_buffet_customer_type` (
    ...
    `sale_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '原始单价（单人，折前价）。自助餐顾客类型原价,下单后价格不受后台改变',
    `sale_price_no_tax`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '销售价,未含税价格（折前）',
    -- 价格计算相关
    `open_overall_discount` INT(10) NOT NULL DEFAULT 0 COMMENT '是否开启整单折扣, 0-否 1-是',
    `price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '最终单价（折后价），只进行自定义打折，不进行会员打折',
    `custom_discount_rate` DECIMAL(20, 4) NOT NULL DEFAULT 1 COMMENT '自定义折扣率, 值为0-1之间(0-100%)',
    `custom_discount_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '自定义折扣金额（单人）。自定义折扣金额（单人）=自助餐顾客类型原价*自定义折扣率',
    `tax_rate`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '税率,值为0-1之间.加购时记录税率,结账时再重新核算',
    `service_tax_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '服务费税费（单人）,0-不收取税费；收取时，服务费税费=服务费*税率',
    `tax_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '自助餐顾客类型税费（单人）。自助餐顾客类型已含税时，税费=自助餐顾客类型原价*(1-1/(1+税率))；自助餐顾客类型未含税时，税费=自助餐顾客类型原价*税率',
    `service_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '服务费（单人）,0-固定服务费 大于0-按比例收服务费；自助餐顾客类型已含税时，服务费=(自助餐顾客类型原价-自助餐顾客类型税费)*服务费比例；自助餐顾客类型未含税时，服务费=自助餐顾客类型原价*服务费比例',
    `total_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '应收金额(单人)。商品已含税时，应收金额(单人)=(最终单价-商品税费)+服务费+总税费；商品未含税时，应收金额(单商品)=最终单价+服务费+总税费',
    `origin_total_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '原始应收金额(单人)。商品已含税时，应收金额(单人)=（原始单价-商品税费)+服务费+总税费；商品未含税时，应收金额(单商品)=原始单价+服务费+总税费',
...
CREATE TABLE IF NOT EXISTS `ttpos_material` (
    ...
    `price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '采购单价',
    `stock_num`  DECIMAL(20, 4) UNSIGNED NOT NULL DEFAULT 0.0000 COMMENT '库存数量',
...
CREATE TABLE IF NOT EXISTS `ttpos_material_category` (
    ...
    `name` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '原料分类名称',
    `multi_language_name_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '多语言名称ID',
    `create_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建时间(时间戳)',
    `update_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '更新时间(时间戳)',
    `delete_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '删除时间(时间戳)',
...
CREATE TABLE IF NOT EXISTS `ttpos_purchase_order_item` (
    ...
    `material_name` TEXT NOT NULL COMMENT '物品名称JSON, 提交采购时记录后不再修改',
...
CREATE TABLE IF NOT EXISTS `ttpos_product_package` (
    ...
    `price` DECIMAL(12, 2) NOT NULL DEFAULT 0 COMMENT '套餐价格',
    `product_type` INT(10) NOT NULL DEFAULT 0 COMMENT '商品类型, 0-商品 1-套餐',
...
CREATE TABLE IF NOT EXISTS `ttpos_product_bom` (
    ...
    `purchase_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '采购单价',
    `price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '价格',
    ...
    `product_package_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '商品包ID',
    `product_bom_card_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '成本卡ID',
    `stock_num`  DECIMAL(20, 4) NOT NULL DEFAULT 0.0000 COMMENT '库存数量',
    `barcode_value` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '条形码值',
...
CREATE TABLE IF NOT EXISTS `ttpos_related_material` (
    ...
    `num` DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '材料用量,可小数',
    `unit_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '单位ID,物品单位',
    `unit_name` TEXT NOT NULL COMMENT '单位名称JSON,物品单位名称',
    `base_unit_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '基准单位ID,物品基准单位',
    `base_unit_name` TEXT NOT NULL COMMENT '基准单位名称JSON,物品基准单位名称',
...
CREATE TABLE IF NOT EXISTS `ttpos_member` (
    ...
    `point`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '积分',
    `frozen_point`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '冻结积分。冻结积分不能使用，在前端显示为已扣除或已增加。冻结积分可为负数。积分余额=积分+冻结积分',
    `accumulated_consumption_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '累计消费金额',
    `consumption_count` INT(11) NOT NULL DEFAULT 0 COMMENT '消费次数',
    `balance`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '余额',
    `frozen_balance`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '冻结余额。冻结余额不能使用，在前端显示为已扣除或已增加。冻结余额可为负数。会员余额=余额+冻结余额',
    `gift_balance`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '赠送账户余额',
    `frozen_gift_balance`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '冻结赠送账户余额。冻结赠送账户余额不能使用，在前端显示为已扣除或已增加。冻结赠送账户余额可为负数。赠送账户余额=赠送账户余额+冻结赠送账户余额',
    `accumulated_recharge_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '累计充值金额',
...
CREATE TABLE IF NOT EXISTS `ttpos_member_level` (
    ...
    `upgrade_money`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '升级条件，累计消费额',
    `open_point` INT(10) DEFAULT 0 COMMENT '是否开放累计积分升级，0-否 1-是',
    `upgrade_point`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '升级条件，累计积分',
    `discount` DECIMAL(20, 4) NOT NULL DEFAULT 1 COMMENT '等级权益,百分比折扣,单位%, 如80%为打8折，discount值为0.8 ',
...
CREATE TABLE IF NOT EXISTS `ttpos_member_card_type` (
    ...
    `price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '价格',
    `discount` DECIMAL(20, 4) NOT NULL DEFAULT 1 COMMENT '折扣,单位%',
    `sort` INT(11) NOT NULL DEFAULT 0 COMMENT '排序',
    `status` INT(10) NOT NULL DEFAULT 0 COMMENT '状态, 0-开启 1-关闭',
    `open_point` INT(10) NOT NULL DEFAULT 0 COMMENT '开卡赠送积分,0-否 1-是',
    `open_point_num`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '开卡赠送积分数',
    `open_money` INT(10) NOT NULL DEFAULT 0 COMMENT '开卡赠送余额,0-否 1-是',
    `open_money_num`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '开卡赠送余额数',
...
CREATE TABLE IF NOT EXISTS `ttpos_member_card` (
    ...
    `discount`  DECIMAL(20, 4) NOT NULL DEFAULT 1 COMMENT '折扣,单位%, 如80%为打8折，discount值为0.8 .不随后台改变,按领取时的折扣。后续会员卡类型折扣改变时,不改变此字段',
...
CREATE TABLE IF NOT EXISTS `ttpos_member_card_log` (
    ...
    `price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '价格,会员卡价格,不随后台改变,记录领取时的价格',
    `discount` DECIMAL(20, 4) NOT NULL DEFAULT 1 COMMENT '折扣,单位%,不随后台改变,记录领取时的折扣',
    ...
    `member_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '会员ID',
    `give_money`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '赠送余额',
    `give_point`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '赠送积分',
    `create_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建时间(时间戳)',
    `update_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '更新时间(时间戳)',
    `delete_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '删除时间(时间戳)',
    UNIQUE KEY `unique_uuid` (`uuid`)
...
CREATE TABLE IF NOT EXISTS `ttpos_member_balance_log` (
    ...
    `sale_order_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '销售订单ID',
    `money`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '变动金额,负数:减余额 正数:加余额。包含赠送余额',
    `gift_money`  DECIMAL(20, 4) DEFAULT 0.00 COMMENT '变动赠送金额',
...
CREATE TABLE IF NOT EXISTS `ttpos_member_point_log` (
    ...
    `value`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '数值,负数:减积分 正数:加积分',
    `describe` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '变动描述',
...
CREATE TABLE IF NOT EXISTS `ttpos_member_recharge_order` (
    ...
    `amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '交易金额=充值金额+手续费',
    `refund_money`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '退款金额，不大于amount',
    `charge_due`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '找零',
    `recharge_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '充值金额',
    `refund_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '退款充值金额，不大于recharge_amount',
    `gift_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '赠送金额',
    `gift_point`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '赠送积分',
    ...
    `balance`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '充值前会员余额',
    `balance_recharged`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '充值后会员余额',
    `create_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建时间(时间戳)',
    `update_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '更新时间(时间戳)',
    `delete_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '删除时间(时间戳)',
...
CREATE TABLE IF NOT EXISTS `ttpos_warehouse_form` (
    ...
    `num` DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '数量',
    `remark` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '备注',
...
CREATE TABLE IF NOT EXISTS `ttpos_warehouse_form_item` (
    ...
    `num` DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '入库数量',
...
CREATE TABLE IF NOT EXISTS `ttpos_purchase_form` (
    ...
    `num`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '总数量',
    `amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '总金额',
...
CREATE TABLE IF NOT EXISTS `ttpos_purchase_form_item` (
    ...
    `estimate_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '预计单价',
    `estimate_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '预计金额',
    `num` INT(11) NOT NULL DEFAULT 0 COMMENT '数量',
    `price`  DECIMAL(20, 4) NOT NULL DEFAULT 0 COMMENT '单价',
...
CREATE TABLE IF NOT EXISTS `ttpos_staff_shift_log` (
    ...
    `previous_shift_cash`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '上一班遗留备用金',
    `current_cash_total`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '当前钱箱现金总计',
    `incomes` TEXT  COMMENT '收入详情',
    `total_income`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '总收入',
    `cash_taken_out`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '本班取出现金',
    `cash_left`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '本班遗留备用金',
    `cash_income`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '本班收入现金',
    `total_business`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '本班营业总额(不包含退款)',
    `is_printed` INT(10) NOT NULL DEFAULT 0 COMMENT '是否打印 0-未打印 1-已打印',
    `remark` VARCHAR(255) DEFAULT NULL COMMENT '备注',
    `withdraw_cash`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '中途取出现金',
    `deposit_cash`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '中途存入现金',
...
CREATE TABLE IF NOT EXISTS `ttpos_cashier_duty_detail` (
    ...
    `total_sales`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '总销售额',
    `total_service_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '总服务费',
    `total_payment_commission_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '总支付手续费',
    `total_tax_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '总税费',
    `total_product_quantity` INT(11) NOT NULL DEFAULT 0 COMMENT '商品数量',
    `total_discount_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '总优惠折扣',
    `total_refund_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '总退款',
    `total_revenue`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '总营业收入',
    `total_actual_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '总实收金额',
    `total_recharge_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '充值金额',
    `total_gift_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '赠送金额',
    `total_gift_point` INT(11) NOT NULL DEFAULT 0 COMMENT '赠送积分',
    `previous_balance`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '上一班遗留备用金',
    `total_off_cash_withdrawal`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '下班取出现金',
    `total_cash_balance`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '本班遗留备用金',
    `cash_deposit`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '中途存入现金',
    `cash_withdrawal`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '中途取出现金',
    ...
    `total_min_order_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '最小订单金额',
    `total_max_order_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '最大订单金额',
    `total_average_order_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '平均订单金额',
    `total_table_customer_count` INT(11) NOT NULL DEFAULT 0 COMMENT '桌台人数',
    `total_table_min_order_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '桌台最小订单金额',
    `total_table_max_order_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '桌台最大订单金额',
    `total_table_average_order_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '桌台人均消费金额',
    `total_scan_order_count` INT(11) NOT NULL DEFAULT 0 COMMENT '点餐订单数',
    `total_scan_min_order_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '点餐最小订单金额',
    `total_scan_max_order_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '点餐最大订单金额',
    `total_scan_average_order_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '点餐平均订单金额',
    `total_gift_product_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '赠菜金额',
...
CREATE TABLE IF NOT EXISTS `ttpos_return_order` (
    ...
    `refund_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '退款金额,包括税额',
    `unit` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '货币单位',
    `refund_tax_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '退款税额',
...
CREATE TABLE IF NOT EXISTS `ttpos_return_order_amount` (
    ...
    `amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '退款金额',
...
CREATE TABLE IF NOT EXISTS `ttpos_return_order_product` (
    ...
    `tax_rate`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '税率,根据结账时税率计算',
    `num` DECIMAL(12, 8) NOT NULL DEFAULT 0.00000000 COMMENT '商品数量,退货的商品数量',
    `product_discount` DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '商品折扣',
    `product_total_amount` DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '商品总金额（退款总金额）',
    `erp_code` VARCHAR(255) NOT NULL DEFAULT '' COMMENT 'ERP系统商品编码',
    `create_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建时间(时间戳)',
    `update_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '更新时间(时间戳)',
    `delete_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '删除时间(时间戳)',
...
CREATE TABLE IF NOT EXISTS `ttpos_cash_box` (
    ...
    `name` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '名称',
    `balance`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '钱箱余额',
    `frozen_balance`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '冻结金额。冻结金额不能使用，在前端显示为已扣除或已增加。冻结金额可为负数。钱箱余额=钱箱余额+冻结金额',
    `previous_balance`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '上一班遗留备用金',
    `cash_withdrawal`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '中途取出金额',
    `cash_deposit`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '中途存入金额',
    `create_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建时间(时间戳)',
    `update_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '更新时间(时间戳)',
    `delete_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '删除时间(时间戳)',
    UNIQUE KEY `unique_uuid` (`uuid`)
...
CREATE TABLE IF NOT EXISTS `ttpos_cash_box_log` (
    ...
    `amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '金额',
    `remark` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '备注',
...
CREATE TABLE IF NOT EXISTS `ttpos_statistics_sale` (
    ...
    `product_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '商品原价: 不含税',
    `product_origin_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '原商品金额',
    `product_sale_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '商品销售价',
    `product_num`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '商品数量',
    `product_tax`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '商品税',
    `service_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '服务费',
    `service_tax`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '服务税',
    `discount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '优惠折扣',
    `discount_member`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '会员折扣',
    `gift_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '赠菜金额',
    `gift_num`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '赠菜数量',
    `free_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '免单金额',
    `free_num`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '免单数量',
    `payment_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '支付金额',
    `payment_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '支付手续费',
    `payment_balance`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '支付余额',
    `refund_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '退款金额',
    `refund_payment_balance`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '退款支付余额',
    `refund_tax`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '退款税额',
    `no_refund_tax`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '不退税金额',
    `extend_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '扩展价格',
    ...
    `delivery_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '配送费',
    `refund_service_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '退款服务费',
    `refund_discount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '退款优惠折扣',
    `refund_discount_member`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '退款会员折扣',
    `refund_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '退款支付手续费',
...
CREATE TABLE IF NOT EXISTS `ttpos_statistics_product` (
    ...
    `product_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '商品单价: 未含税',
    `product_sale_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '商品销售价: 规格+加料',
    `product_final_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '商品最终价',
    `flavor_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '商品原价(规格价)',
    `sauce_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '加料价格',
    `product_num`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '商品数量',
    `tax_rate`  DECIMAL(20, 4) NOT NULL DEFAULT 0.0000 COMMENT '税率',
    `tax_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '税费',
    `service_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '服务费',
    `service_tax`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '服务税',
    `give_num`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '赠菜数量',
    `free_num`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '免单数量',
    `refund_num`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '退款数量',
    `is_takeout` INT(10) NOT NULL DEFAULT 0 COMMENT '是否外送',
    `member_order_discount_rate`  DECIMAL(20, 4) NOT NULL DEFAULT 1.0000 COMMENT '会员端商品价格上浮比例1%-300%',
    `complete_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '完成时间',
...
CREATE TABLE IF NOT EXISTS `ttpos_statistics_customer_type` (
    ...
    `buffet_package_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '自助餐套餐ID',
    `buffet_customer_type_price_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '自助餐客户类型价格ID',
    `product_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '销售价,未含税价格（折前）',
    `product_sale_price`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '原始单价（单人，折前价）。自助餐顾客类型原价,下单后价格不受后台改变',
    `product_num`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '商品数量',
    `tax_rate`  DECIMAL(20, 4) NOT NULL DEFAULT 0.0000 COMMENT '税率',
    `tax_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '税费',
    `service_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '服务费',
...
CREATE TABLE IF NOT EXISTS `ttpos_statistics_member` (
    ...
    `duty_no` varchar(255) NOT NULL DEFAULT '' COMMENT '当班编号',
    `recharge_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '充值金额',
    `give_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '赠送金额',
    `give_point`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '赠送积分',
    `payment_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '支付金额',
    `payment_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '支付手续费',
    `refund_amount`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '退款金额',
    `refund_fee`  DECIMAL(20, 4) NOT NULL DEFAULT 0.00 COMMENT '退款手续费',
    `complete_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '完成时间',
    `refund_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '完成时间',
    `create_time` int(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建时间',
    `update_time` int(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS `ttpos_marketing_activity` (
  ...
  `reward_condition_amount`  DECIMAL(20, 4) DEFAULT 0.00 COMMENT '奖励条件金额',
...
CREATE TABLE IF NOT EXISTS `ttpos_marketing_activity_consumption` (
  ...
  `consumption_amount`  DECIMAL(20, 4) DEFAULT 0.00 COMMENT '消费金额',
  `reward_amount`  DECIMAL(20, 4) DEFAULT 0.00 COMMENT '奖励金额',
...
CREATE TABLE IF NOT EXISTS `ttpos_marketing_coupon` (
  ...
  `amount`  DECIMAL(20, 4) DEFAULT 0.00 COMMENT '优惠券金额',
...
CREATE TABLE IF NOT EXISTS `ttpos_member_coupon` (
  ...
  `valid_start_time` int(11) DEFAULT 0 COMMENT '优惠券有效开始时间, requirement = none 时有效',
  `valid_end_time` int(11) DEFAULT 0 COMMENT '优惠券有效结束时间, requirement = none 时有效',
  `amount`  DECIMAL(20, 4) DEFAULT 0.00 COMMENT '优惠券面值',
...
CREATE TABLE IF NOT EXISTS `ttpos_member_coupon_use_record` (
  ...
  `use_order_amount`  DECIMAL(20, 4) DEFAULT 0.00 COMMENT '优惠券使用订单金额',
  `create_time` int(11) DEFAULT 0 COMMENT '创建时间',
  `update_time` int(11) DEFAULT 0 COMMENT '更新时间',
  `delete_time` int(11) DEFAULT 0 COMMENT '删除时间',
  PRIMARY KEY (`id`)
