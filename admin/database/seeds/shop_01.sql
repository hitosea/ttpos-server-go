...
CREATE TABLE IF NOT EXISTS `ttpos_sale_order_product` (
    ...
    -- 价格信息
    `flavor_price` DECIMAL(12, 2) NOT NULL DEFAULT 0 COMMENT '规格原价,仅某规格商品的原价',
    `sauce_price` DECIMAL(12, 2) NOT NULL DEFAULT 0 COMMENT '小料价,所有小料的价格之和',
    `product_price` DECIMAL(12, 2) NOT NULL DEFAULT 0 COMMENT '原始单价,规格原价+小料价',
    `is_custom_price` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否自定义价格, 0-否 1-是',
    `sale_price` DECIMAL(12, 2) NOT NULL DEFAULT 0 COMMENT '销售价,当自定义价格时，销售价=自定义价格,否则销售价=原始单价',
    -- 税率
    ...
    -- 最终单价=销售价*折扣率
    `price` DECIMAL(12, 2) NOT NULL DEFAULT 0 COMMENT '最终单价(会员、会员卡和优惠折扣后)。销售价*折扣率',
     -- 总税费=商品税费+服务费税费
    `service_tax_fee` DECIMAL(12, 2) NOT NULL DEFAULT 0 COMMENT '服务费税费,0-不收取税费；收取时，服务费税费=服务费*税率',
    `tax_fee` DECIMAL(12, 2) NOT NULL DEFAULT 0 COMMENT '商品税费。商品已含税时，税费=规格原价*(1-1/(1+税率))；商品未含税时，税费=原始单价*税率',
    -- 服务费
    `service_fee` DECIMAL(12, 2) NOT NULL DEFAULT 0 COMMENT '服务费,0-固定服务费 大于0-按比例收服务费；商品已含税时，服务费=(最终单价-商品税费)*服务费比例；商品未含税时，服务费=最终单价*服务费比例',
   -- 单个商品应收金额=最终单价+服务费+总税费
    ...
    `qrcode_order_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '扫码订单ID，用于关联扫码订单，用于判断是否为扫码订单商品',
    `is_qrcode_order_product` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否为扫码订单商品, 0-否 1-是',
    `is_accept_order` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否已接单, 0-否 1-是',
    -- 时间信息
    `create_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建时间(时间戳)',
    `update_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '更新时间(时间戳)',
    `delete_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '删除时间(时间戳)',
...
CREATE TABLE IF NOT EXISTS `ttpos_sale_order_product_bom` (
    ...
    `flavor_name` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '规格或小料规格名称,不随后台更新',
...
CREATE TABLE IF NOT EXISTS `ttpos_member_balance_log` (
    ...
    `member_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '会员ID',
    `scene` TINYINT(2) NOT NULL DEFAULT 0 COMMENT '场景,10-用户充值 20-用户消费 30-管理员操作 40-订单退款 50-余额提现 60-订单反结账 70-充值反结账 80-充值退款 90-扣减',
...
CREATE TABLE IF NOT EXISTS `ttpos_member_point_log` (
    ...
    `value` INT(11) NOT NULL DEFAULT 0 COMMENT '数值,负数:减积分 正数:加积分',
    `describe` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '变动描述',
    `create_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建时间(时间戳)',
    `update_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '更新时间(时间戳)',
    `delete_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '删除时间(时间戳)',
