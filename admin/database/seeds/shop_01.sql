...
CREATE TABLE IF NOT EXISTS `ttpos_h5_order_product` (
    ...
    `num` DECIMAL(12, 2) NOT NULL DEFAULT 0.00 COMMENT '最终商品数量.接单和拒单后从sale_order_product表获取，不再改变',
...
CREATE TABLE IF NOT EXISTS `ttpos_product_bom` (
    ...
    `product_package_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '商品包ID',
    `product_bom_card_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '成本卡ID',
    `stock_num` DECIMAL(14, 4) NOT NULL DEFAULT 0.0000 COMMENT '库存数量',
    `barcode_value` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '条形码值',
...
CREATE TABLE IF NOT EXISTS `ttpos_member` (
    ...
    `frozen_point` DECIMAL(14, 2) NOT NULL DEFAULT 0 COMMENT '冻结积分。冻结积分不能使用，在前端显示为已扣除或已增加。冻结积分可为负数。积分余额=积分+冻结积分',
    `accumulated_consumption_amount` DECIMAL(14, 2) NOT NULL DEFAULT 0 COMMENT '累计消费金额',
    `consumption_count` INT(11) NOT NULL DEFAULT 0 COMMENT '消费次数',
    `balance` DECIMAL(14, 2) NOT NULL DEFAULT 0 COMMENT '余额',
    `frozen_balance` DECIMAL(14, 2) NOT NULL DEFAULT 0 COMMENT '冻结余额。冻结余额不能使用，在前端显示为已扣除或已增加。冻结余额可为负数。会员余额=余额+冻结余额',
    `gift_balance` DECIMAL(14, 2) NOT NULL DEFAULT 0 COMMENT '赠送账户余额',
    `frozen_gift_balance` DECIMAL(14, 2) NOT NULL DEFAULT 0 COMMENT '冻结赠送账户余额。冻结赠送账户余额不能使用，在前端显示为已扣除或已增加。冻结赠送账户余额可为负数。赠送账户余额=赠送账户余额+冻结赠送账户余额',
...
CREATE TABLE IF NOT EXISTS `ttpos_member_card_type` (
    ...
    `open_point_num` decimal(14, 2) NOT NULL DEFAULT 0.00 COMMENT '开卡赠送积分数',
    `open_money` INT(10) NOT NULL DEFAULT 0 COMMENT '开卡赠送余额,0-否 1-是',
    `open_money_num` decimal(14, 2) NOT NULL DEFAULT 0.00 COMMENT '开卡赠送余额数',
...
CREATE TABLE IF NOT EXISTS `ttpos_member_card_log` (
    ...
    `member_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '会员ID',
    `give_money` DECIMAL(14, 2) NOT NULL DEFAULT 0 COMMENT '赠送余额',
    `give_point` DECIMAL(14, 2) NOT NULL DEFAULT 0 COMMENT '赠送积分',
    `create_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建时间(时间戳)',
    `update_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '更新时间(时间戳)',
    `delete_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '删除时间(时间戳)',
    UNIQUE KEY `unique_uuid` (`uuid`)
...
CREATE TABLE IF NOT EXISTS `ttpos_member_balance_log` (
    ...
    `sale_order_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '销售订单ID',
    `money` decimal(14, 2) NOT NULL DEFAULT 0.00 COMMENT '变动金额,负数:减余额 正数:加余额。包含赠送余额',
    `gift_money` decimal(14, 2) DEFAULT 0.00 COMMENT '变动赠送金额',
...
CREATE TABLE IF NOT EXISTS `ttpos_cash_box` (
    ...
    `frozen_balance` DECIMAL(12, 2) NOT NULL DEFAULT 0.00 COMMENT '冻结金额。冻结金额不能使用，在前端显示为已扣除或已增加。冻结金额可为负数。钱箱余额=钱箱余额+冻结金额',
    `previous_balance` DECIMAL(12, 2) NOT NULL DEFAULT 0.00 COMMENT '上一班遗留备用金',
...
CREATE TABLE IF NOT EXISTS `ttpos_statistics_sale` (
    ...
    `product_num` DECIMAL(14, 2) NOT NULL DEFAULT 0.00 COMMENT '商品数量',
    ...
    `gift_num` DECIMAL(14, 2) NOT NULL DEFAULT 0.00 COMMENT '赠菜数量',
    ...
    `refund_fee` DECIMAL(14, 2) NOT NULL DEFAULT 0.00 COMMENT '退款支付手续费',
...
CREATE TABLE IF NOT EXISTS `ttpos_statistics_customer_type` (
    ...
    `buffet_package_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '自助餐套餐ID',
    `buffet_customer_type_price_uuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '自助餐客户类型价格ID',
    `product_price` DECIMAL(14, 2) NOT NULL DEFAULT 0.00 COMMENT '销售价,未含税价格（折前）',
    `product_sale_price` DECIMAL(14, 2) NOT NULL DEFAULT 0.00 COMMENT '原始单价（单人，折前价）。自助餐顾客类型原价,下单后价格不受后台改变',
    `product_num` DECIMAL(12, 2) NOT NULL DEFAULT 0.00 COMMENT '商品数量',
    `tax_rate` DECIMAL(14, 4) NOT NULL DEFAULT 0.0000 COMMENT '税率',
    `tax_fee` DECIMAL(14, 2) NOT NULL DEFAULT 0.00 COMMENT '税费',
    `service_fee` DECIMAL(14, 2) NOT NULL DEFAULT 0.00 COMMENT '服务费',
    `service_tax` DECIMAL(14, 2) NOT NULL DEFAULT 0.00 COMMENT '服务税',
    `give_num` DECIMAL(12, 2) NOT NULL DEFAULT 0.00 COMMENT '赠菜数量',
    `free_num` DECIMAL(12, 2) NOT NULL DEFAULT 0.00 COMMENT '免单数量',
    `refund_num` DECIMAL(12, 2) NOT NULL DEFAULT 0.00 COMMENT '退款数量',
    `complete_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '完成时间',
    `refund_time` INT(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '完成时间',
    `create_time` int(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '创建时间',
    `update_time` int(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT '更新时间',
