...
CREATE TABLE IF NOT EXISTS `ttpos_takeout_order_item_modifier` (
    ...
    `ttpos_modifier_uuid` bigint unsigned NOT NULL DEFAULT 0 COMMENT 'TTPOS修饰符UUID(关联后：规格/加料/属性值的UUID/套餐商品组UUID)',
    ...
    `ttpos_product_uuid` bigint unsigned NOT NULL DEFAULT 0 COMMENT 'TTPOS商品UUID(关联ttpos_product_package.uuid)',
    `ttpos_flavor_uuid` bigint unsigned NOT NULL DEFAULT 0 COMMENT 'TTPOS规格UUID(commodity类型对应product_package_group_item.product_bom_uuid)',
