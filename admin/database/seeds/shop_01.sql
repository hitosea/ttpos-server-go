...
CREATE TABLE IF NOT EXISTS `ttpos_sale_order_product` (
    ...
    `is_h5_order_product` INT(10) NOT NULL DEFAULT 0 COMMENT '是否为扫码订单商品, 0-否 1-是',
...
CREATE TABLE IF NOT EXISTS `ttpos_member` (
    ...
    `frozen_balance` DECIMAL(12, 2) NOT NULL DEFAULT 0 COMMENT '冻结余额。冻结余额不能使用，在前端显示为已扣除或已增加。冻结余额可为负数。会员余额=余额+冻结余额',
    `gift_balance` DECIMAL(12, 2) NOT NULL DEFAULT 0 COMMENT '赠送账户余额',
    `frozen_gift_balance` DECIMAL(12, 2) NOT NULL DEFAULT 0 COMMENT '冻结赠送账户余额。冻结赠送账户余额不能使用，在前端显示为已扣除或已增加。冻结赠送账户余额可为负数。赠送账户余额=赠送账户余额+冻结赠送账户余额',
