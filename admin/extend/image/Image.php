...
class Image
{
    ...
    protected function __construct(\SplFileInfo $file)
    {
        ...
        if ($width * $height > 5000 * 5000) { // 限制图像尺寸为 5000x5000
            throw new ImageException(__('图像尺寸过大'));
        }
        if ('gif' == $this->info['type']) {
            $this->gif = new Gif($file->getPathname());
            $this->im  = @imagecreatefromstring($this->gif->image());
        } else {
            if (!in_array($this->info['type'], ['jpg', 'jpeg', 'png'])) {
                throw new ImageException('仅支持JPG、JPEG、PNG格式');
            }
    ...
    /**
     * 获取图像
     * @return imagecreatetruecolor|GdImage
     */
