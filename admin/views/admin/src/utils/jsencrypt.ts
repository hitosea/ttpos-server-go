 ...
 * @return {Promise}
 */
export async function encrypt(content: any, publicKey: any) {
  try {
    // 假设你要加密的内容
 ...
 * @returns {Promise}
 */
export async function decrypt(content: any, privateKey: any) {
  try {
    ...
    const symmetricKey: any = decrypt.decrypt(btoa(encryptedKey));
    // 将 Base64 解码
    const encryptedDataBytes = CryptoJS.enc.Base64.parse(contents[0]);
    // 获取 IV 和密文
    const iv = CryptoJS.lib.WordArray.create(encryptedDataBytes.words.slice(0, 4)); // 16字节 IV
    const ciphertext = CryptoJS.lib.WordArray.create(encryptedDataBytes.words.slice(4)); // 剩余部分为密文，不取后面的256位数据
    // 使用相同的算法解密
    const decrypted = CryptoJS.AES.decrypt({ ciphertext } as unknown as string, CryptoJS.enc.Utf8.parse(symmetricKey), {
      iv: iv,
      mode: CryptoJS.mode.CBC,
      padding: CryptoJS.pad.Pkcs7,
    });
 ...
 * @returns {Promise}
 */
export async function generateKeys(clientId: any) {
  try {
