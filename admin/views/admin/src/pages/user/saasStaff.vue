<template>
  <div class="p-4 bg-white rounded min-h-full">
    <div class="flex justify-between">
      ...
      <div class="pb-4">
        <el-button type="primary" icon="Plus" :loading="formLoading" @click="handleAdd">{{ $t('添加账号') }}</el-button>
      </div>
    </div>
    <!-- 表格 -->
    <el-table v-loading="isFetching" :data="data?.data?.list?.data" row-key="uuid" border>
      ...
      <el-table-column fixed="right" :label="$t('操作')" width="150">
        <template #default="scope">
          <el-button type="primary" link :loading="formLoading && staffId == scope.row.uuid" @click="handleEdit(scope.row)">
            {{ $t('编辑') }}
          </el-button>
        </template>
      </el-table-column>
