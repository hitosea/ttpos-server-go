...
import jbcImg from '@/assets/jbc.ico';
...
export async function setupPermissions(router) {
    ...
    router.beforeEach(async (to, from, next) => {
        ...
        if (cloudBasic.value.base) {
            ...
            if (window.config instanceof Function && window.config().brand.toUpperCase() == 'TTPOS') {
                iconLink.href = tigerImg;
                document.title = 'TTPOS';
            }
            if (window.config instanceof Function && window.config().brand.toUpperCase() == 'JBC') {
                iconLink.href = jbcImg;
                document.title = 'JBCレジ';
            }
        }
