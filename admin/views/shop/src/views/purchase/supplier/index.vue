    ...
    <div class="common-seach-wrap">
      <el-form size="small" :inline="true" :model="searchForm" class="demo-form-inline">
        <el-form-item :label="$t('供应商名称')">
          <el-input size="small" v-model="searchForm.name" :placeholder="$t('供应商名称')" @input="onSearch"></el-input>
        </el-form-item>
        <el-form-item>
      ...
      <div class="table-wrap">
        <el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column fixed="right" :label="$t('操作')" width="120">
            <template #default="scope">
              <el-button class="button-p" @click="editClick(scope.row)" type="primary" link size="small" v-auth="'/purchase/supplier/edit'">{{ $t('编辑') }}</el-button>
              <el-button class="button-p" @click="deleteClick(scope.row)" type="primary" link size="small" v-auth="'/purchase/supplier/delete'">{{ $t('删除') }}</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
