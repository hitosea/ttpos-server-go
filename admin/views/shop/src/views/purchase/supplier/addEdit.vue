<template>
  <el-dialog :title="title" v-model="dialogVisible" @close="dialogFormVisible" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form size="small" :model="form" label-position="top" ref="form">
      <el-form-item for="no_click" :label="$t('供应商名称')" prop="name" :rules="[{ required: true, message: $t('请输入供应商名称') }]">
        <el-input v-model="form.name" :placeholder="$t('请输入供应商名称')" :maxlength="100" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item for="no_click" :label="$t('供应商地址')" prop="address" :rules="[{ required: true, message: $t('请输入供应商地址') }]">
        <el-input v-model="form.address" :placeholder="$t('请输入供应商地址')" :maxlength="100" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item for="no_click" :label="$t('联系人')" prop="contact_person">
        <el-input v-model="form.contact_person" :placeholder="$t('请输入联系人')" :maxlength="100" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item for="no_click" :label="$t('联系电话')" prop="contact_phone">
        <el-input v-model="form.contact_phone" :placeholder="$t('请输入联系电话')" :maxlength="50" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item for="no_click" :label="$t('职位')" prop="position">
        <el-input v-model="form.position" :placeholder="$t('请输入职位')" :maxlength="100" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item for="no_click" :label="$t('采购负责人')" prop="purchaser_id" :rules="[{ required: true, message: $t('请选择采购负责人') }]">
        <el-select size="small" v-model="form.purchaser_id" filterable :placeholder="$t('请选择采购负责人')">
