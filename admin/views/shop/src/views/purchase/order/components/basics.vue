<template>
  <div class="basic-setting-content">
    ...
    <el-form-item for="no_click" :label="$t('采购名称')" :rules="[{ required: true, message: $t('请输入采购名称') }]" prop="name">
      <el-input :placeholder="$t('请输入采购名称')" :disabled="readOnly || change" v-model="form.name" class="max-w460"></el-input>
    </el-form-item>
    <el-form-item for="no_click" :label="$t('申请人')" :rules="[{ required: true, message: $t('请选择申请人') }]" prop="applicant_id">
    ...
    <el-form-item for="no_click" :label="$t('到货时间')" :rules="[{ required: true, message: $t('请选择时间') }]" prop="arrival_time">
      <el-date-picker
        size="small"
    ...
    <el-form-item for="no_click" v-if="sType == '20' && level == '2'" :label="$t('采购方式')" :rules="[{ required: true, message: $t('请选择采购方式') }]" prop="type">
