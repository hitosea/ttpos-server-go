  ...
  <div class="purchase-box">
    <el-form size="small" ref="form" class="purchase-form" :model="form" label-position="top" label-width="180px">
      <div class="common-button-wrapper">
        <el-button size="small" class="return-button" link @click="cancelFunc"
          ><el-icon> <ArrowLeftBold /> </el-icon>{{ $t('返回') }}</el-button
        >
      </div>
      <div class="purchase-form-flex" ref="formContainer">
        ...
        <!-- 采购明细 -->
        <detail :readOnly="true" :detail="true"></detail>
        ...
        PurchaseApi.detailErpPurchaseOrder(Params, true)
          .then((data) => {
            ...
            self.form.purchase_detail.map((item, index) => {
              self.form.purchase_detail[index].product = {
                type: 10,
                image: [{ file_path: '' }],
                product_name_text: '',
              };
              self.form.purchase_detail[index].product.type = item.product_type;
              self.form.purchase_detail[index].product.product_name_text = item.product_name || '';
              self.form.purchase_detail[index].product.image[0].file_path = item.productImage?.file_path || '';
            });
            self.activities = [];
            self.form.logs.map((item) => {
              self.activities.push({
                content: item.username + ' ' + $t('操作') + ' ' + $t(item.operation) + ' ' + (item.status == 20 ? `(${$t('驳回原因')}：${item.remark})` : ''),
