<template>
  <el-dialog class="" @close="handleClose" v-model="dialogVisible" :close-on-click-modal="false"
    ...
    <el-form size="small" ref="form" :model="form" label-position="top" :rules="rules">
      <!-- <el-form-item for="no_click"  v-if="have" :label="$t('原密码')" prop="cashier_password"
                :rules="[{ required: true, message: $t('请输入原密码') }]">
                <el-input type="password" v-model="form.cashier_password"></el-input>
            </el-form-item> -->
      <el-form-item for="no_click" :label="have ? $t('新密码') : $t('密码')" prop="new_cashier_password"
      ...
      <el-form-item for="no_click" :label="$t('确认密码')" prop="confirm_cashier_password">
  ...
  methods: {
    onSubmit() {
      ...
      self.$refs.form.validate((valid) => {
        if (!valid) return;
        self.loading = true;
        switch (this.openTitle) {
          case $t('修改钱箱密码'): {
            Terminal.editCashierPassword(
              {
                new_cashier_password: self.form.new_cashier_password,
                confirm_cashier_password: self.form.confirm_cashier_password,
              },
              true
            )
              .then((data) => {
          ...
          case $t('修改锁屏密码'): {
            Terminal.editCashierLockPassword(
              {
                new_lock_password: self.form.new_cashier_password,
                confirm_lock_password: self.form.confirm_cashier_password,
              },
              true
            )
              .then((data) => {
          ...
          case $t('修改高级密码'): {
            Terminal.editCashierAdvancedPassword(
              {
                new_advanced_password: self.form.new_cashier_password,
                confirm_advanced_password: self.form.confirm_cashier_password,
              },
              true
            )
              .then((data) => {
