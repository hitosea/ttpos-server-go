<template>
    <el-dialog class="" @close="handleClose" v-model="dialogVisible" :close-on-click-modal="false" :close-on-press-escape="false" :title="$t('修改高级密码')">
        <el-form size="small" ref="form" :model="form" label-position="top" :rules="rules">
            ...
            <el-form-item for="no_click" :label="have ? $t('新密码') : $t('密码')" prop="new_advanced_password">
                <el-input v-model="form.new_advanced_password" type="password" :placeholder="$t('请输入密码')"></el-input>
                <div class="tips">{{ $t('密码必须是4-8位的数字') }}</div>
            </el-form-item>
            <el-form-item for="no_click" :label="$t('确认密码')" prop="confirm_advanced_password">
