<template>
  ...
  <div class="user">
      ...
      <el-form size="small" :inline="true" :model="searchForm" class="demo-form-inline">
        ...
        <el-form-item label="配送方式">
          <el-select size="small" v-model="searchForm.deliver_source" placeholder="请选择">
            ...
            <el-option v-for="(item, index) in deliver_source" :key="index" :label="item.name" :value="item.value"> </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-table size="small" :data="tableData.data" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column fixed="right" label="操作" width="120">
            <template #default="scope">
              <el-button @click="detailClick(scope.row)" type="primary" link size="small" v-auth="'/takeout/deliver/detail'">详情 </el-button>
              <el-button
              ...
              <el-button
                ...
                v-if="scope.row.deliver_source == 10 && scope.row.status == 10"
                type="primary"
                link
                size="small"
    ...
    <Cancel v-if="open_edit" :open_edit="open_edit" :deliver_id="deliver_id" :order_no="order_no" @closeDialog="closeDialogFunc($event, 'edit')"> </Cancel>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      onExport: function () {
        ...
        window.location.href = baseUrl + '/index.php/shop/takeout.deliver/export?' + qs.stringify(this.searchForm);
      },
      /*关闭弹窗*/
      closeDialogFunc(e, f) {
        if (f == 'edit') {
