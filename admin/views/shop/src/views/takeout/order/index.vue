<template>
  ...
  <div class="user">
      ...
      <div class="table-wrap">
        <el-tabs v-model="activeName" @tab-change="handleClick">
          <el-tab-pane label="全部订单" name="all">
            <template #label>
              <span
                >全部订单 <el-tag size="">{{ order_count.all }}</el-tag></span
              >
            </template>
          </el-tab-pane>
          <el-tab-pane :label="'未付款'" name="payment">
            <template #label>
              <span
                >未付款 <el-tag size="">{{ order_count.payment }}</el-tag></span
              >
            </template>
          </el-tab-pane>
          <el-tab-pane :label="'进行中'" name="process">
            <template #label>
              <span
                >进行中 <el-tag size="">{{ order_count.process }}</el-tag></span
              >
            </template>
          </el-tab-pane>
          <el-tab-pane :label="'已取消'" name="cancel">
            <template #label>
              <span
                >已取消 <el-tag size="">{{ order_count.cancel }}</el-tag></span
              >
            </template>
          </el-tab-pane>
          <el-tab-pane :label="'已完成'" name="complete">
            <template #label>
              <span
                >已完成 <el-tag size="">{{ order_count.complete }}</el-tag></span
              >
            </template>
          </el-tab-pane>
        </el-tabs>
        <el-table size="small" :data="tableData.data" :span-method="arraySpanMethod" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column fixed="right" label="操作" width="200">
            <template #default="scope">
              <div v-if="!scope.row.is_top_row">
                <el-button @click="addClick(scope.row)" type="primary" link size="small" v-auth="'/takeout/order/detail'">订单详情 </el-button>
                <el-button
                ...
                <el-button
                  v-if="
                    scope.row.deliver_source == 10 &&
                    scope.row.order_status.value == 10 &&
                    scope.row.delivery_type.value == 10 &&
                    scope.row.pay_status.value == 20 &&
                    scope.row.delivery_status.value == 20
                  "
                  @click="verifyClick(scope.row)"
                  type="primary"
                  link
    ...
    <!--处理-->
    <refund v-if="open_refund" :open_edit="open_refund" :order_no="order_no" :order_id="order_id" @closeDialog="closerefundDialogFunc($event, 'edit')"> </refund>
  </div>
</template>
    ...
    methods: {
      ...
      verifyClick(row) {
        ...
        ElMessageBox.confirm('是否确认此操作?', '提示', {
          ...
          .then(() => {
            ...
            OrderApi.takeExtract(extract_form, true)
              .then((data) => {
                self.loading = false;
                this.$ElMessage({
                  message: $t('操作成功'),
      ...
      senDada(row) {
        ...
        ElMessageBox.confirm('是否确认此操作?', '提示', {
          ...
          .then(() => {
            extract_form.order_id = row.order_id;
            OrderApi.sendDada(extract_form, true)
              .then((data) => {
                self.loading = false;
                this.$ElMessage({
                  message: $t('操作成功'),
