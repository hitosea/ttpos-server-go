  ...
  <div class="product-list">
    <div class="common-level-rail">
      <el-form size="small" :inline="true" :model="searchForm" class="demo-form-inline">
        <el-form-item>
          <el-input size="small" v-model="searchForm.name" :placeholder="$t('标签名称')" @input="onSearch"></el-input>
        </el-form-item>
        <el-form-item>
      ...
      <div class="table-wrap">
        <el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading" @selection-change="handleSelectionChange">
          ...
          <el-table-column fixed="right" :label="$t('操作')" width="240">
            <template #default="scope">
              <el-button @click="editClick(scope.row)" type="primary" link size="small" v-auth="'/product/expand/label/edit'">{{ $t('编辑') }} </el-button>
              <el-button @click="relatedProductClick(scope.row)" type="primary" v-auth="'/product/expand/label/relatedProduct'" link size="small">{{ $t('关联商品') }} </el-button>
              <el-button
                @click="deleteClick(scope.row.label_id)"
                type="primary"
                link
                size="small"
                v-auth="'/product/expand/label/delete'"
...
<script setup>
  ...
  const getData = async () => {
    const params = {
      ...
      label_name: searchForm.name,
    };
    loading.value = true;

    try {
      const data = await ProductApi.LabelList(params, true);
      loading.value = false;
  ...
  const handleProductSelectorClose = async (list) => {
    if (Array.isArray(list)) {
      try {
        loading.value = true;
        await ProductApi.relateByLabel(
          {
            label_id: model.value.label_id,
