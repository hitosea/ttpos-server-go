  ...
  <div class="product-wrapper">
    ...
    <div class="product-list">
      ...
      <div class="common-level-rail">
        ...
        <div>
          <el-button size="small" type="primary" v-auth="'/product/expand/attr/edit'" :disabled="erp_is_open == 1" @click="handleOpenGroupManager">{{
            $t('管理属性组')
          }}</el-button>
          <el-button size="small" v-auth="'/product/expand/attr/batch_delete'" :disabled="multipleSelection.length == 0 || erp_is_open == 1" @click="deleteBatch">{{
            $t('批量删除')
          }}</el-button>
          <el-button size="small" type="primary" icon="Plus" v-auth="'/product/expand/attr/add'" :disabled="erp_is_open == 1" @click="addClick">{{ $t('添加属性') }}</el-button>
        </div>
      </div>
      <!--内容-->
        ...
        <div class="table-wrap">
          <el-table size="small" :data="attributeTableData" border style="width: 100%" v-loading="loading" @selection-change="handleSelectionChange">
            ...
            <el-table-column fixed="right" :label="$t('操作')" width="240">
              <template #default="scope">
                <el-button @click="editClick(scope.row)" type="primary" link size="small" :disabled="erp_is_open == 1" v-auth="'/product/expand/attr/edit'">{{
                  $t('编辑')
                }}</el-button>
                <el-button @click="relatedProductClick(scope.row)" v-auth="'/product/expand/attr/relatedProduct'" type="primary" link size="small" :disabled="erp_is_open == 1"
                  >{{ $t('关联商品') }}
                </el-button>
                <el-button
