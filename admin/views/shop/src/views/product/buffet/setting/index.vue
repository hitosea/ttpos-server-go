      ...
      <!-- <el-form-item :label="$t('自助餐优惠折扣')" :rules="[{ required: true, message: '' }]">
            ...
            message: $t('请输入优惠名称')
                                    ...
                                    <el-form-item label="" style="margin-bottom: 4px !important;" :rules="[{ required: true, message: '' }]"
                                        @change="handleChangeDiscountType(item, index)">
                                    ...
                                    <el-form-item class="favorable-box-item" label=""
                                        :prop="item.discount_type == '1' ? `add_buffet_discount[${index}].discount_ratio` : `add_buffet_discount[${index}].discount_price`"
                                        :rules="[{ required: true, validator: () => { return (item.discount_type == '1' ? item.discount_ratio : item.discount_price) ? true : false; }, message: item.discount_type == '1' ? $t('请输入比例') : $t('请输入优惠金额') }]">
                                        <p class="p-unit" v-if="item.discount_type == '1'&& currency.unit_position == '0'">%</p>
                                        <p class="p-unit" v-if="item.discount_type == '2' && currency.unit_position == '0'">{{ currency.unit }}</p>
                                        ...
                                        <p class="p-unit" v-if="item.discount_type == '1'&& currency.unit_position == '1'">%</p>
                                        <p class="p-unit" v-if="item.discount_type == '2' && currency.unit_position == '1'">{{ currency.unit }}</p>
                                    </el-form-item>
                                    <el-form-item label="" :prop="`add_buffet_discount[${index}].buffet_ids`"
                                        :rules="[{ required: true, validator: () => { return item.buffet_ids.length > 0 ? true : false; }, message: $t('请选择自助餐') }]">
                                            ...
                                            <template v-for="items, indexs in select_list[index]">
                                                <div class="select-button">
                                                    ...
                                                    <p class="price-p" v-if="item.discount_type == '1'">
                                                        {{ this.$formatPrice((Number(items.price) * Number(item.discount_ratio / 100)).toFixed(2)) }}
                                                    </p>
