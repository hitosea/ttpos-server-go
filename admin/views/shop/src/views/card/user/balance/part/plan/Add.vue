<template>
  ...
  <el-dialog title="添加充值套餐" v-model="dialogVisible" @close='dialogFormVisible' :close-on-click-modal="false"
    ...
    <el-form size="small" :model="form" label-position="top" ref="form">
      ...
      <el-form-item label="到账金额" :label-width="formLabelWidth" prop="real_money" :rules="[{required: true,message: '请输入到账金额'}]">
        <el-input type="number" placeholder="请输入到账金额" v-model="form.real_money">
        </el-input>
      </el-form-item>
      <el-form-item label="排序" :label-width="formLabelWidth" prop="sort" :rules="[{required: true,message: '接近0，排序等级越高'}]">
        <el-input type="number" placeholder="接近0，排序等级越高" v-model="form.sort">
        </el-input>
      </el-form-item>
    </el-form>
    <template #footer>
