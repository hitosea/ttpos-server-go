<template>
  <div class="user">
    ...
    <div class="common-level-rail">
      <el-button size="small" type="primary" @click="addClick" icon="Plus" v-auth="'/card/user/grade/add'">{{ $t('添加等级') }}</el-button>
    </div>
      ...
      <div class="table-wrap">
        <el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column fixed="right" :label="$t('操作')" width="120">
            <template #default="scope">
              <el-button @click="editClick(scope.row)" type="primary" link size="small" v-auth="'/card/user/grade/edit'">{{ $t('编辑') }}</el-button>
              <el-button @click="deleteClick(scope.row)" type="primary" link size="small" v-if="scope.row.is_default == 0" v-auth="'/card/user/grade/delete'">{{
                $t('删除')
              }}</el-button>
            </template>
          </el-table-column>
