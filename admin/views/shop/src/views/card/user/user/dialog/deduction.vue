<template>
  ...
  <el-dialog :title="$t('扣减')" v-model="dialogVisible" @close="dialogFormVisible" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form size="small" ref="formRef" :model="form" label-position="top">
      ...
      <el-form-item for="no_click" :label="form.source == '0' ? $t('当前赠送账户余额') : $t('当前积分')">
        <el-input v-if="form.source == '0'" v-model="editData.gift_balance" autocomplete="off" disabled></el-input>
        <el-input v-else v-model="editData.points" autocomplete="off" disabled></el-input>
      </el-form-item>

      <el-form-item
        for="no_click"
        :label="form.source == '0' ? $t('扣减金额') : $t('扣减数量')"
        prop="value"
      ...
      >
        <el-input-number
          :controls="false"
          :min="0"
          :max="form.source == '0' ? Number(editData.gift_balance) : Number(editData.points)"
          :placeholder="form.source == '0' ? $t('请输入扣减金额') : $t('请输入扣减数量')"
          :precision="2"
