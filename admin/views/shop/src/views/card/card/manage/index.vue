  ...
  <div class="user">
    ...
    <div class="common-seach-wrap flex">
      ...
      <div class="common-level-rail">
        <el-button size="small" type="primary" @click="addClick" icon="Plus" v-auth="'/card/card/add'">{{ $t('添加会员卡') }} </el-button>
      </div>
    </div>
    <!--内容-->
      ...
      <div class="table-wrap">
        <el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column fixed="right" :label="$t('操作')" width="200">
            <template #default="scope">
              <el-button @click="putClick(scope.row)" type="primary" link size="small" :disabled="scope.row.status != 0" v-auth="'/card/card/put'" v-if="scope.row.type_id != 3">{{
                $t('发卡')
              }}</el-button>
              <el-button @click="editClick(scope.row)" type="primary" link size="small" v-auth="'/card/card/edit'">{{ $t('编辑') }} </el-button>
              <el-button @click="deleteClick(scope.row)" type="primary" link size="small" v-auth="'/card/card/delete'">{{ $t('删除') }} </el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /*打开添加*/
      addClick() {
        this.$router.push('/' + this.app_id + '/card/card/add');
      },
      /*打开编辑*/
      editClick(item) {
        this.$router.push({
          path: '/' + this.app_id + '/card/card/edit',
          query: {
