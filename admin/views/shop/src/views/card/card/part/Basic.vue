<template>
  <div class="basic-setting-content pl16 pr16">
    ...
    <el-form-item for="no_click" :label="$t('会员卡名称：')" :rules="[{ required: true, message: $t('请输入会员卡名称') }]" prop="model.card_name">
      <el-input v-model="form.model.card_name" :maxlength="50" class="max-w460" :placeholder="$t('请输入会员卡名称')"></el-input>
    </el-form-item>
    <!-- <el-form-item for="no_click"  :label="$t('卡片类型：')" prop="model.is_default">
      <el-radio-group v-model="form.model.is_default">
          <el-radio :label="0">{{ $t('默认') }}</el-radio>
          <el-radio :label="1">{{ $t('自定义') }}</el-radio>
        </el-radio-group>
    </el-form-item>
    <el-form-item for="no_click"  :label="$t('卡片样式：')" prop="model.card_style" v-if="form.model.is_default==0">
    ...
    <el-form-item for="no_click" :label="$t('卡片样式：')" prop="card_style" v-if="form.model.is_default == 1">
      <div class="d-s-c f-w maxwidth-530">
    ...
    <el-form-item for="no_click" :label="$t('排序：')" :rules="[{ required: true, message: $t('请输入排序') }]" prop="model.sort">
    ...
    <el-form-item for="no_click" :label="$t('会员权益：')">
      <el-radio-group v-model="form.model.is_discount">
        <el-radio :label="0">{{ $t('无折扣') }}</el-radio>
        <el-radio :label="1">{{ $t('有折扣') }}</el-radio>
      </el-radio-group>
    </el-form-item>
    <el-form-item for="no_click" :label="$t('折扣：')" :rules="[{ required: true, message: $t('请输入折扣') }]" prop="model.discount" v-if="form.model.is_discount == 1">
    ...
    <el-form-item for="no_click" :label="$t('开卡赠送：')">
    ...
    <el-form-item for="no_click" :label="$t('优惠券：')" :rules="[{ required: true, message: $t('请输入优惠券') }]" prop="model.open_coupons" v-if="form.model.open_coupon">
      <div class="d-s-c">
        <div style="width: 460px; border: 1px solid #dedede; min-height: 32px"
          ><span v-for="(item, index) in form.model.open_coupons" :key="index">{{ item.name + ';' }}</span>
        </div>
        <span></span>
        <el-button type="primary" @click="addCoupon()">{{ $t('添加优惠券') }}</el-button>
      </div>
    </el-form-item>
    <el-form-item for="no_click" label="" v-if="form.model.open_coupon">
      <el-table :data="form.model.open_coupons" style="width: 60%">
        ...
        <el-table-column prop="number" :label="$t('每人领取数量')" :rules="[{ required: true, message: ' ' }]">
          <template #default="scope">
            <el-input type="number" v-model="scope.row.number" placeholder=""> </el-input>
          </template>
        </el-table-column>
