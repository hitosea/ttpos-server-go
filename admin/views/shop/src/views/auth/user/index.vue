<template>
  <div class="user">
    ...
    <div class="common-level-rail">
      <el-button size="small" type="primary" icon="Plus" @click="addClick" v-auth="'/auth/user/add'">{{ $t('添加管理员') }}</el-button>
    </div>
      ...
      <div class="table-wrap">
        <el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column prop="role.role_name" :label="$t('角色')">
            <template #default="scope">
              <div v-if="scope.row.is_super == 0">
                <span class="mr10" v-for="(item, index) in scope.row.userRole" :key="index">{{ item.role.role_name }}</span>
              </div>
          ...
          <el-table-column prop="is_status" :label="$t('状态')" width="80">
            <template #default="scope">
              <el-switch
                ...
                :model-value="scope.row.is_status"
                :active-value="0"
                :inactive-value="1"
                @click="isShowFunc(scope.row)"
              ></el-switch>
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" :label="$t('操作')" width="120">
            <template #default="scope">
              <el-button v-if="scope.row.is_super < 1" @click="editClick(scope.row)" type="primary" link size="small" v-auth="'/auth/user/edit'">{{ $t('编辑') }}</el-button>
              <el-button
    ...
    <Add v-if="open_add" :open="open_add" :roleList="roleList" @close="closeAdd"></Add>

    <Edit v-if="open_edit" :open="open_edit" :roleList="roleList" :shop_user_id="curModel.shop_user_id" @close="closeEdit"></Edit>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /*是否显示开关*/
      isShowFunc(e) {
        if (!(e.is_super < 1 || (e.is_super == 1 && e.user_type == 1)) || !this.$filter.isAuth('/auth/user/state')) {
          return;
        }
