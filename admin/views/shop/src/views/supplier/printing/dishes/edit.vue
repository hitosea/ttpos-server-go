...
<script>
  ...
  export default {
    name: 'SupplierPrintingDishesEdit',
    components: {
      ProductSelector,
    },
    data() {
      ...
      getData() {
        ...
        SupplierApi.getEditPrinting({ id: self.id }, true)
          .then((data) => {
            ...
            if (self.form.product_method == 2) {
              self.printProductsDataByPrintTag = Array.isArray(data.data.model.product_ids) ? data.data.model.product_ids : [];
            }
            ...
            this.form.printer_id.map((item, index) => {
