<template>
  <el-dialog class="" @close="handleClose" v-model="dialogVisible" :close-on-click-modal="false" :close-on-press-escape="false" :title="$t('整单备注')">
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        let self = this;
        self.loading = true;
        SettingApi.getOrderRemark({}, true)
          .then((res) => {
            const data = res.data;
            if (!Array.isArray(data)) return;

            self.form.remark = data.map((item) => {
              const getOrderRemark = (value) => {
                try {
              ...
              return {
                id: item.id,
                action: 'edit',
                data: getOrderRemark(item.order_remark),
              };
            });
          })
          .catch((err) => {})
      ...
      onSubmit() {
        ...
        self.$refs.formRef.validate((valid) => {
          ...
          SettingApi.setOrderRemark({ remark }, true)
            .then((res) => {
              this.$ElMessage({
                type: 'success',
                message: $t('更新成功'),
