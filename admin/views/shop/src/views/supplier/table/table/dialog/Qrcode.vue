  ...
  export default {
    ...
    methods: {
      ...
      qrcodeClick() {
        ...
        link.download = `${this.code_name}_qrcode.png`; // 设置下载文件名
      ...
      modifyQRCode() {
        ...
        img.onload = () => {
          ...
          // 测量文本宽度
          const textWidth = ctx.measureText(self.code_name).width;

          let displayText = self.code_name;

          // 如果文本宽度超过最大允许宽度，则截断并添加省略号
          if (textWidth > maxWidth) {
            displayText = self.code_name.slice(0, 8) + '...';
          }
