<template>
  <div class="product">
    <div class="common-level-rail">
      <el-button size="small" type="primary" @click="addClick" icon="Plus" v-auth="'/supplier/table/area/add'">{{ $t('添加区域') }}</el-button>
    </div>
        ...
        <el-table size="small" :data="tableData" row-key="category_id" style="width: 100%" v-loading="loading">
          ...
          <el-table-column fixed="right" :label="$t('操作')" width="100">
            <template #default="scope">
              <el-button @click="editClick(scope.row)" type="primary" link size="small" v-auth="'/supplier/table/area/edit'">{{ $t('编辑') }}</el-button>
              <el-button @click="deleteClick(scope.row)" type="primary" :disabled="scope.row.can_delete == 0" link size="small" v-auth="'/supplier/table/area/delete'">{{
                $t('删除')
              }}</el-button>
            </template>
          </el-table-column>
