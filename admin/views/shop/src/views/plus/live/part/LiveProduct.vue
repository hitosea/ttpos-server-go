<template>
  ...
  >
      ...
      <div class="table-wrap">
        <el-table size="small" :data="productList" border style="width: 100%" highlight-current-row v-loading="loading" @selection-change="tableCurrentChange">
          ...
          <el-table-column width="140" prop="onSale" label="上架状态">
            <template #default="scope">
              <el-switch @change="onSaleChange($event, scope.row)" v-model="scope.row.on_sale" :active-value="1" :inactive-value="0"></el-switch>
            </template>
          </el-table-column>
          <el-table-column prop="isPush" width="180" label="推送状态">
            <template #default="scope">
              <el-button :disabled="scope.row.isPush == 0" @click="pushChange(scope.row)" size="small" type="primary">推送</el-button>
    ...
    <ImportProduct v-if="open_import" :open_import="open_import" :islist="true" :room_id="room_id" @closeDialog="closePop" @success="initGetData"></ImportProduct>
  </el-dialog>
</template>
