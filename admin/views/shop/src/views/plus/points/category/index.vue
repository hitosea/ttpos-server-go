  ...
  <div class="product">
    <div class="common-level-rail">
      <el-button size="small" type="primary" @click="addClick" icon="Plus"
        v-auth="'/product/takeaway/category/Add'">添加分类</el-button>
    </div>
    <!--内容-->
        ...
        <el-table size="small" :data="tableData" row-key="category_id" default-expand-all
          ...
          <el-table-column prop="" label="图片" width="180">
            <template #default="scope">
              <img v-img-url="scope.row.image.file_path" alt="" :width="50" />
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" label="操作" width="100">
            <template #default="scope">
              <el-button @click="editClick(scope.row)" type="primary" link size="small"
                v-auth="'/plus/points/category/edit'">编辑</el-button>
              <el-button @click="deleteClick(scope.row)" type="primary" link size="small"
                v-auth="'/plus/points/category/delete'">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        let self = this;
        self.loading = true;
        PointsApi.categoryList({}, true)
          .then(data => {
            self.loading = false;
            self.tableData = data.data.list.all;
