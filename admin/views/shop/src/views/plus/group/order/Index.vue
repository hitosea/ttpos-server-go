      ...
      <div class="table-wrap">
        <el-tabs v-if="user_type == 0" v-model="activeName" @tab-change="handleClick">
          ...
          <el-tab-pane :label="'待使用'" name="process">
            <template #label>
              <span
                >待使用 <el-tag size="">{{ order_count.process }}</el-tag></span
              >
            </template>
          </el-tab-pane>
          ...
          <el-tab-pane :label="'已退款'" name="refund">
            <template #label>
              <span
                >已退款 <el-tag size="">{{ order_count.refund }}</el-tag></span
              >
            </template>
          </el-tab-pane>
        ...
        <el-table size="small" :data="tableData.data" :span-method="arraySpanMethod" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column fixed="right" label="操作" width="160">
            <template #default="scope">
              <div v-if="!scope.row.is_top_row">
                <el-button @click="addClick(scope.row)" type="primary" link size="small" v-auth="'/store/order/detail'">订单详情 </el-button>
              </div>
            </template>
          </el-table-column>
        </el-table>
