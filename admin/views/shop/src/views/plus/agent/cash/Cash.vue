          ...
          <el-table-column fixed="right" label="操作" width="180">
            <template #default="scope">
              <div v-if="scope.row.apply_status.value == 10 || scope.row.apply_status.value == 20">
                <el-button @click="editClick(scope.row)" type="primary" link size="small" v-auth="'/plus/agent/cash/submit'">审核 </el-button>
                <template v-if="scope.row.apply_status.value == 20 && scope.row.pay_type.value != 10">
                  <el-button @click="makeMoney(scope.row)" type="primary" link size="small" v-auth="'/plus/agent/cash/money'"> 确认打款</el-button>
                </template>
                <template v-if="scope.row.apply_status.value == 20 && scope.row.pay_type.value == 10">
                  <el-button @click="WxPay(scope.row.id)" type="primary" link size="small" v-auth="'/plus/agent/cash/money'"> 微信付款</el-button>
                </template>
              </div>
