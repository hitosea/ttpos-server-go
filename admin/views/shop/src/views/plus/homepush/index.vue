<template>
  ...
  <div class="pb50">
    <el-form size="small" ref="form" :model="form" label-position="top" label-width="100px">
      ...
      <div class="huodong">建议换活动，此名称更换，商城首页弹窗根据此名称不同来弹窗</div>
      <div v-if="form.is_open">
        ...
        <div class="huodong">图片为2M以内大小，格式限png，jpg格式
        ...
        <el-form-item label="跳转地址" v-if="form.type < 3" :rules="[{ required: true, message: ' ' }]" prop="link">
          <el-input class="max-w460" placeholder="点击选择你要链接的位置" v-if="form.link" :value="'链接到' + form.link.type + ' ' + form.link.name" :disabled="true"></el-input>
