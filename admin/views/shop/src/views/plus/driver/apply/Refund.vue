<template>
  ...
  <div class="user">
      ...
      <div class="table-wrap">
        <el-table :data="tableData" size="small" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column fixed="right" label="操作" width="50">
            <template #default="scope">
              <div>
                <el-button v-if="scope.row.apply_status.value == 10" @click="editClick(scope.row)" type="primary" link size="small" v-auth="'/plus/driver/apply/refundStatus'">
                  审核
                </el-button>
              </div>
            </template>
    ...
    <Auth v-if="open_edit" :open_edit="open_edit" :form="userModel" @closeDialog="closeDialogFunc($event, 'edit')"> </Auth>
  </div>
</template>
