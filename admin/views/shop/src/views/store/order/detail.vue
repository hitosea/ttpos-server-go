<template>
  <div class="pb50" v-loading="loading">
            ...
            <template #default="scope">
              <div>
                ...
                <span class="tips" v-if="Number(scope.row.refund_amount) > 0">
                  （{{ $t('退款：') + currency.unit + $formatPrice(scope.row.refund_amount) }}）
                </span>
              </div>
            </template>
          </el-table-column>
    ...
    <refund v-if="openRefund" :open_edit="openRefund" :order_no="orderNo" :order_id="saleBillUuid" :sub_order_id="saleOrderUuid" :pay_price="payPrice" @closeDialog="e => closerefundDialogFunc(e, 'edit')" />
