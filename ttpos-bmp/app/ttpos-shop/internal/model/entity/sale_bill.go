...
type SaleBill struct {
	...
	IsSplitOrder          int     `json:"isSplitOrder"          orm:"is_split_order"          description:"是否拆单, 0-否 1-是"`                                                                       // 是否拆单, 0-否 1-是
	...
	Amount                float64 `json:"amount"                orm:"amount"                  description:"订单金额(折后价),关联销售订单的总金额之和"`                                                              // 订单金额(折后价),关联销售订单的总金额之和
	OriginAmount          float64 `json:"originAmount"          orm:"origin_amount"           description:"订单金额(折前价)。商品未含税时，订单金额(折前价)=商品金额+服务费+税费。商品已含税时，订单金额(折前价)=商品金额（含商品消费税）+服务费+税费（只有服务费税）"` // 订单金额(折前价)。商品未含税时，订单金额(折前价)=商品金额+服务费+税费。商品已含税时，订单金额(折前价)=商品金额（含商品消费税）+服务费+税费（只有服务费税）
	...
	LockTime              int     `json:"lockTime"              orm:"lock_time"               description:"锁单时间"`                                                                                // 锁单时间
