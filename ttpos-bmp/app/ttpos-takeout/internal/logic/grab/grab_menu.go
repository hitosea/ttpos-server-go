...
//   - err: 错误信息
func (s *sGrab) BatchUpdateMenuItems(ctx context.Context, req *api.BatchUpdateMenuReq) (*api.BatchUpdateMenuResp, error) {
	g.Log().Infof(ctx, "[Grab] 批量更新菜单: shopUUID=%s, field=%s, count=%d",
	...
	if err != nil {
		g.Log().Errorf(ctx, "[Grab] 获取 merchant_id 失败: shopUUID=%s, error=%v", req.ShopUuid, err)
		return nil, gerror.Wrap(err, "获取 merchant_id 失败")
	}
	...
	if req.Field != grabDto.MenuItemUpdateFieldItem && req.Field != grabDto.MenuItemUpdateFieldModifier {
		return nil, gerror.Newf("field 必须是 ITEM 或 MODIFIER，当前值: %s", req.Field)
	}
	if len(req.MenuEntities) == 0 {
		return nil, gerror.New("menu_entities 不能为空")
	}
	if len(req.MenuEntities) > 200 {
		return nil, gerror.Newf("menu_entities 数量不能超过 200 个，当前: %d", len(req.MenuEntities))
	}

	// 3. Proto → SDK 请求转换
	sdkReq, err := convertProtoToSDK(ctx, req, merchantID)
	if err != nil {
		g.Log().Errorf(ctx, "[Grab] 转换请求参数失败: %v", err)
		return nil, gerror.Wrap(err, "转换请求参数失败")
	}

	// 5. 调用 Grab API
	sdkResp, err := service.Grab().BatchUpdateMenu(ctx, merchantID, sdkReq)
	if err != nil {
		// 6. 记录失败日志
		s.logBatchUpdate(ctx, merchantID, req.Field, len(req.MenuEntities), false, err.Error())

		g.Log().Errorf(ctx, "[Grab] 批量更新菜单 API 调用失败: merchantID=%s, field=%s, error=%v",
			merchantID, req.Field, err)
		return nil, gerror.Wrap(err, "调用 Grab BatchUpdateMenu API 失败")
	}

	// 7. 转换 SDK 响应到 Proto
	protoResp := convertSDKRespToProto(ctx, sdkResp, req.ShopUuid)

	// 8. 记录成功日志
	success := protoResp.Status == "success"
	errMsg := ""
	if !success {
		// 记录错误摘要
		errMsg = fmt.Sprintf("部分失败: %d 个错误", len(protoResp.Errors))
	}
	s.logBatchUpdate(ctx, merchantID, req.Field, len(req.MenuEntities), success, errMsg)

	g.Log().Infof(ctx, "[Grab] 批量更新菜单完成: merchantID=%s, field=%s, status=%s, errorCount=%d",
		merchantID, req.Field, protoResp.Status, len(protoResp.Errors))

	return protoResp, nil
}

// convertProtoToSDK 转换 Proto BatchUpdateMenuReq 到 SDK BatchUpdateMenuItem
func convertProtoToSDK(ctx context.Context, protoReq *api.BatchUpdateMenuReq, merchantID string) (*grabfood.BatchUpdateMenuItem, error) {
	// 1. 创建 SDK 请求
	sdkReq := grabfood.NewBatchUpdateMenuItem(merchantID, protoReq.Field)

	// 2. 转换 MenuEntities
	var sdkUpdates []grabfood.MenuEntity
	for i, protoEntity := range protoReq.MenuEntities {
		// 2.1 创建 SDK MenuEntity
		sdkEntity := grabfood.NewMenuEntity()
		sdkEntity.SetId(protoEntity.Id)

		// 2.2 设置可选字段 - Price
		if protoEntity.Price != nil {
			sdkEntity.SetPrice(*protoEntity.Price)
		}

		// 2.3 设置可选字段 - AvailableStatus
		if protoEntity.AvailableStatus != nil {
			sdkEntity.SetAvailableStatus(*protoEntity.AvailableStatus)
		}

		// 2.4 设置可选字段 - MaxStock（仅商品支持）
		if protoReq.Field == grabDto.MenuItemUpdateFieldItem && protoEntity.MaxStock != nil {
			sdkEntity.SetMaxStock(*protoEntity.MaxStock)
		}

		// 2.5 转换高级定价配置
		if len(protoEntity.AdvancedPricings) > 0 {
		...
		// 2.6 转换购买能力配置（仅商品支持）
		if protoReq.Field == grabDto.MenuItemUpdateFieldItem && len(protoEntity.Purchasabilities) > 0 {
		...
		g.Log().Debugf(ctx, "[Grab] 转换菜单实体 [%d]: id=%s, price=%v, status=%v",
			i, protoEntity.Id, protoEntity.Price, protoEntity.AvailableStatus)
	}

	// 3. 设置 MenuEntities
	sdkReq.SetMenuEntities(sdkUpdates)

	g.Log().Infof(ctx, "[Grab] SDK 请求构建完成: merchantID=%s, field=%s, updates=%d",
		merchantID, protoReq.Field, len(sdkUpdates))
	return sdkReq, nil
}

// convertSDKRespToProto 转换 SDK BatchUpdateMenuResponse 到 Proto
func convertSDKRespToProto(ctx context.Context, sdkResp *grabfood.BatchUpdateMenuResponse, shopUUID string) *api.BatchUpdateMenuResp {
	...
	if len(sdkResp.GetErrors()) > 0 {
		protoResp.Errors = make([]*api.MenuEntityError, 0, len(sdkResp.GetErrors()))
