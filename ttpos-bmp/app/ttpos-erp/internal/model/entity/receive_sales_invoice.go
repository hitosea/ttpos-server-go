...
// ReceiveSalesInvoice is the golang structure for table receive_sales_invoice.
type ReceiveSalesInvoice struct {
	...
	SaleOrderUuid     string `json:"saleOrderUuid"     orm:"sale_order_uuid"     description:"TTPOS订单UUID（幂等键）"`                      // TTPOS订单UUID（幂等键）
	...
	SalesInvoiceName  string `json:"salesInvoiceName"  orm:"sales_invoice_name"  description:"ERP Sales Invoice名称"`                   // ERP Sales Invoice名称
	PaymentEntryNames string `json:"paymentEntryNames" orm:"payment_entry_names" description:"ERP Payment Entry名称(JSON)"`             // ERP Payment Entry名称(JSON)
