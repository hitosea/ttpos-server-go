...
// AdminUserColumns defines and stores column names for the table ttpos_admin_user.
type AdminUserColumns struct {
	AdminUserId string // 自增ID
	Username    string // 用户名
	Phone       string // 手机号
	Password    string // 登录密码
	RealName    string // 姓名
	IsSuper     string // 是否超级管理员
	Status      string // 状态(0未启用,1已启用)
	CreateTime  string // 创建时间（时间戳）
	UpdateTime  string // 更新时间（时间戳）
	DeleteTime  string // 删除时间（时间戳）
}

// adminUserColumns holds the columns for the table ttpos_admin_user.
