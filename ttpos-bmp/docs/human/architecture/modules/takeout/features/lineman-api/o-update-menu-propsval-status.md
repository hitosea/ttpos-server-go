...
**Name:** Update Menu Property Values Status API
...
| Name | Type | Required | Description |
| --- | --- | --- | --- |
| propertyValues | Array | M | 门店属性值状态列表<br>**备注：** 最大数组长度 100 |
| ...propertyValues[].id | String(255) | M | 属性值 ID（与菜单同步中的 values.id 相同） |
| ...propertyValues[].status | int | M | 属性值状态，可选值：<br>- 1：AVAILABLE（可用）<br>- 2：SOLD_OUT_TODAY（今日售罄，明天自动恢复）<br>- 3：SUSPENDED（暂停） |
