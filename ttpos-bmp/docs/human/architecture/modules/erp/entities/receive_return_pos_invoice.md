# ReceiveReturnPosInvoice 实体模型说明

## 基本信息

- **实体名称**: ReceiveReturnPosInvoice
- **表名**: receive_return_pos_invoice
- **所属模块**: ttpos-erp
- **描述**: 接收退货 POS 发票记录实体，用于记录从 POS 系统接收并发送到 ERPNext 的退货发票请求和响应
...
| PostingDatetime | *gtime.Time | 过账日期时间 | 时间对象 |
...
5. **退货时间**: PostingDatetime 记录退货的时间点
6. **公司关联**: 通过 CompanyAbbr 关联公司信息
...
```go
// 根据订单号查询退货发票记录
returnInvoice, err := dao.ReceiveReturnPosInvoice.Ctx(ctx).
    Where(dao.ReceiveReturnPosInvoice.Columns().OrderNo, orderNo).
    Where(dao.ReceiveReturnPosInvoice.Columns().SiteCode, siteCode).
    One()
```
...
```go
// 查看请求和响应数据
reqData, _ := base64.StdEncoding.DecodeString(returnInvoice.ReqMessage)
respData, _ := base64.StdEncoding.DecodeString(returnInvoice.RespMessage)
```
