...
- ✅ 迁移脚本统一放在 `manifest/sql/` 目录下
- ✅ 通过 `make db_add NAME=[备注]` 生成迁移脚本
- ✅ 脚本内容写在生成的 `xxx_up.sql` 文件内
...
详见：`ttpos-bmp/.cursor/rules/proto-rules.mdc`
...
1. **定义 Protobuf**（如需要）
```bash
cd app/ttpos-erp/manifest/protobuf
# 编辑 .proto 文件
make proto  # 生成代码
```
...
```bash
cd app/ttpos-erp
make db_add NAME=create_users_table
# 编辑生成的 SQL 文件
make migrate  # 执行迁移
```
