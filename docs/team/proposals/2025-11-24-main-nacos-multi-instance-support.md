...
| **状态**   | 已创建 Spec   |
| **关联任务** | -      |
| **关联 Spec** | [task-main-nacos-multi-instance-support](../../shared/specs/task-main-nacos-multi-instance-support/)      |
...
**示例场景**：
> 生产环境部署了 Nacos 集群（3个节点），但当前代码只能连接其中一个节点。当该节点故障时，`ttpos-main` 无法从 Nacos 获取服务发现信息，导致无法调用 `ttpos-bmp` 子模块的 gRPC 服务，整个系统不可用。
