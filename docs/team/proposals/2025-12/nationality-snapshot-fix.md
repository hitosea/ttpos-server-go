...
| **状态**   | 已批准 → Spec 已创建   |
| **关联任务** | -      |
| **关联 Spec** | [story-main-nationality-snapshot-fix](../../shared/specs/archived/v2.12/story-main-nationality-snapshot-fix/) |
...
当前订单查询时，国籍信息会随后台数据变更而改变，导致订单历史信息不准确。这是订单商品信息快照修复需求（`order-attribute-snapshot-fix.md`）的子任务。
...
2. **国籍信息被改名**：
   - 订单国籍："中国"
   - 后台将"中国"改名为"中华人民共和国"
   - 查询订单时显示："中华人民共和国"（显示的是新名称，而非下单时的名称）
...
- ✅ **数据准确性**：确保订单国籍信息准确反映下单时的状态
- ✅ **合规性**：满足财务、税务对订单历史记录的要求
- ✅ **可追溯性**：支持订单历史查询和问题追溯
- ✅ **用户体验**：用户查看历史订单时看到的是下单时的真实国籍信息
...
**核心思路**：订单国籍信息应该使用快照数据，而不是从关联表实时获取。

**现状分析**：

1. **数据库设计缺少快照字段**：
   - `ttpos_sale_bill` 表只有 `nationality_uuid` 字段，关联到 `ttpos_nationality` 表
   - 没有 `nationality_name` 快照字段
   - 查询时从 `Nationality.MultiLanguageName` 获取名称
   - 导致国籍数据被删除或改名时，订单显示信息变化

2. **代码实现依赖关联表**：
   - 订单详情查询中的 `NationalityName` 直接从 `Nationality.MultiLanguageName` 获取
   - 没有快照机制

**解决方案**：

1. **新增快照字段**（需要数据库结构变更）：
   - 在 `ttpos_sale_bill` 表添加 `nationality_name` 字段（VARCHAR(255)），用于快照国籍名称（单语言）

2. **修复查询逻辑**：
   - 优先使用 `SaleBill.NationalityName` 字段（快照）
   - 如果快照字段为空，降级使用 `Nationality.MultiLanguageName`（兼容历史数据）
   - 实现多语言支持：主语言使用快照，其他语言从关联表补充

3. **实施策略**：
   - **新订单**：下单时保存国籍名称快照到 `nationality_name` 字段
...
1. **新增快照字段**
   - 在 `ttpos_sale_bill` 表添加 `nationality_name` 字段
   - 类型：VARCHAR(255)
   - 注释：国籍名称快照（单语言），不随后台更新
   - 默认值：空字符串

#### 二、代码修改

2. **修改数据模型**
   - 在 `SaleBill` 结构体添加 `NationalityName` 字段
   - 字段类型：`string`
   - JSON 标签：`json:"nationality_name"`
   - GORM 标签：`gorm:"column:nationality_name"`

3. **修复国籍获取逻辑**
   - 新增 `SaleBill.GetLocaleNationalityName()` 方法
   - 实现逻辑：
     - 优先使用 `SaleBill.NationalityName` 字段（快照）
...
4. **修改下单逻辑**
   - 在创建订单时，保存国籍名称快照到 `SaleBill.NationalityName`
   - 从 `Nationality.MultiLanguageName.ZhName` 获取中文名称
   - 确保所有下单入口都保存快照数据

5. **修改订单查询逻辑**
   - 在订单详情查询中，使用 `GetLocaleNationalityName()` 方法获取国籍名称
   - 替换原有的直接从关联表获取的逻辑

#### 三、数据迁移和兼容性

6. **数据完整性检查**
   - 检查历史订单的 `nationality_name` 字段填充情况
   - 统计需要迁移的订单数量
...
1. **数据库结构变更**（0.5 天）
   - 设计数据库迁移脚本
   - 添加 `nationality_name` 字段到 `ttpos_sale_bill` 表
   - 执行迁移并验证
...
1. **数据检查**：
   - 先检查历史数据的 `nationality_name` 字段填充情况
   - 根据检查结果决定是否需要数据迁移
...
- 国籍模型: `main/app/model/nationality.go`

### 代码位置

**问题代码**：
- 订单详情查询中的 `NationalityName` 获取逻辑（从 `Nationality.MultiLanguageName` 获取）

**数据模型**：
- `main/app/model/sale_bill.go:15-112` - `SaleBill` 模型定义（需要新增 `NationalityName` 字段）
...
- [x] 创建 Spec：`story-main-nationality-snapshot-fix` ✅ 2025-12-02
- [ ] 分配负责人：{姓名}
- [ ] 目标 Sprint：Sprint {N}
...
1. **WHEN** 查询包含国籍信息的订单 **THEN** 系统 **SHALL** 显示下单时保存的国籍名称快照
2. **IF** 后台删除了某个国籍配置 **THEN** 历史订单 **SHALL** 仍然显示该国籍的原始名称
3. **IF** 后台修改了某个国籍的名称 **THEN** 历史订单 **SHALL** 显示修改前的原始名称
4. **IF** 订单快照数据为空（历史数据） **THEN** 系统 **SHALL** 降级使用关联表数据（兼容性）
5. **WHEN** 创建新订单 **THEN** 系统 **SHALL** 正确保存国籍名称快照到 `SaleBill.NationalityName` 字段
6. **WHEN** 查询订单国籍信息 **THEN** 系统 **SHALL** 返回多语言格式（`LocaleResponse`）
...
5. **修改订单查询逻辑**：
   - 在订单详情查询接口中，使用 `SaleBill.GetLocaleNationalityName()` 获取国籍名称
   - 替换原有的直接从 `Nationality.MultiLanguageName` 获取的逻辑

6. **数据检查脚本**：
   - 检查 `ttpos_sale_bill.nationality_name` 字段的填充率
   - 统计需要迁移的订单数量
