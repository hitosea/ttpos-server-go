...
| 门店切换 | `/api/v1/kiosk/store_switch` | POST | 切换门店，返回新的 token |
...
| 接口名称 | 接口路径 | 请求方法 | 说明 |
|---------|---------|---------|------|
| 获取基本信息 | `/api/v1/kiosk/base` | GET | 获取基本信息（包含轮播广告、语言设置等配置，参考其他终端的 base 接口） |
...
| 模块 | 接口数量 | 说明 |
|-----|---------|------|
| 登录认证模块 | 4 | 登录、刷新Token、退出登录、门店切换（验证码使用全局接口 `/passport/captcha`） |
