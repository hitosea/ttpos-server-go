...
> 本文档定义外卖来源信息快照修复功能的技术设计和实现方案。

## 📋 概述

本功能为订单表（`ttpos_sale_bill`）添加外卖来源名称快照字段（`order_source_name`），确保订单历史信息准确反映下单时的真实外卖来源状态，不随后台配置变更而改变。采用 JSON 方案保存完整多语言数据，既保证快照完整性，又提供完整的多语言支持。

**核心特性**：
- 数据库结构变更：添加 `order_source_name` 快照字段（TEXT 类型，存储 JSON）
...
- ✅ Model 层添加字段和方法（`SaleBill.OrderSourceName` 和 `GetLocaleOrderSourceName()`）
- ✅ 修改下单和查询逻辑，使用快照数据
- ✅ 不使用 panic，返回 error
- ✅ 遵循单一职责原则
...
1. **SaleBill 模型**: `main/app/model/sale_bill.go`
   - 添加 `OrderSourceName` 字段
   - 添加 `GetLocaleOrderSourceName()` 方法
   - 添加 `SetOrderSourceNameSnapshot()` 方法

2. **OrderSource 模型**: `main/app/model/order_source.go`
   - 已有的关联表模型
   - 用于降级查询
...
5. **参考实现**: `main/app/model/sale_bill.go` - `GetLocaleNationalityName()` 和 `SetNationalityNameSnapshot()`
   - 已实现的 JSON 方案快照逻辑
   - 可直接复用实现模式
...
**下单逻辑集成**：
- 在创建 `SaleBill` 时，从 `OrderSource.MultiLanguageName` 获取完整多语言数据
- 序列化为 JSON 保存到 `SaleBill.OrderSourceName` 字段
- 涉及所有下单入口（POS、扫码点餐、外卖等）

**查询逻辑集成**：
- 在订单查询时，使用 `SaleBill.GetLocaleOrderSourceName()` 方法
...
**变更说明**：在 `ttpos_sale_bill` 表添加 `order_source_name` 字段，用于保存外卖来源名称快照（JSON 格式，包含所有语言）。

**迁移 SQL**：

```sql
-- 添加外卖来源名称快照字段（JSON 方案）
...
| 字段 | 类型 | 说明 | 约束 |
|------|------|------|------|
| order_source_name | TEXT | 外卖来源名称快照（JSON，包含所有语言） | NOT NULL, DEFAULT '' |
...
4. **字段位置**: 紧跟 `order_source_uuid` 之后，便于理解关联关系
...
type SaleBill struct {
    ...
    OrderSourceName string `gorm:"column:order_source_name;type:text" json:"order_source_name"` // 新增快照字段（JSON）
    // ... 其他字段
    
    // 关联
    OrderSource *OrderSource `gorm:"foreignKey:OrderSourceUuid;references:Uuid" json:"-"`
}
...
- `OrderSourceName`: 外卖来源名称快照字段（JSON 格式，包含所有语言）
- GORM 标签：`column:order_source_name;type:text` 映射到数据库字段
- JSON 标签：`json:"order_source_name"` 用于 JSON 序列化
- 字段位置：紧跟 `OrderSourceUuid` 之后
...
#### 新增: SetOrderSourceNameSnapshot() 方法
...
1. **GetLocaleOrderSourceName()**:
   - **快照优先**: 优先使用 `OrderSourceName` 快照字段（JSON）
   - **JSON 解析**: 尝试将快照字段反序列化为 `LocaleResponse`
   - **降级逻辑**: 快照为空或解析失败时，降级使用 `OrderSource.MultiLanguageName`（兼容历史数据）
   - **兜底处理**: 关联表为 nil 或已删除时，返回空响应
   - **返回格式**: `dto.LocaleResponse`（多语言响应）

2. **SetOrderSourceNameSnapshot()**:
...
- ✅ 使用 `SetOrderSourceNameSnapshot()` 方法保存快照
- ✅ 从 `OrderSource.MultiLanguageName` 获取完整多语言数据
- ✅ 序列化为 JSON 保存到 `SaleBill.OrderSourceName` 字段
- ✅ 处理边界情况：外卖来源不存在或为空
...
所有查询订单外卖来源名称的地方，使用 `GetLocaleOrderSourceName()` 方法：
...
- ✅ 替换原有的直接从 `OrderSource.MultiLanguageName` 获取的逻辑
- ✅ 使用 `SaleBill.GetLocaleOrderSourceName()` 方法
- ✅ 确保所有查询接口都使用快照数据
- ✅ 验证历史订单兼容性（快照字段为空的情况）
...
#### 测试 GetLocaleOrderSourceName() 方法

```go
// main/app/model/sale_bill_test.go

func TestSaleBill_GetLocaleOrderSourceName(t *testing.T) {
    // 测试场景1: 快照有值（JSON）+ 关联表存在
    t.Run("Snapshot exists (JSON) and relation exists", func(t *testing.T) {
        snapshotJSON := `{"zh":"美团外卖","th":"","en":"Meituan","zhtw":"","ja":"","ko":"","my":"","tr":"","sv":""}`
        saleBill := &SaleBill{
            OrderSourceName: snapshotJSON,
        ...
        result := saleBill.GetLocaleOrderSourceName()
        
        // 应使用快照数据（所有语言）
        assert.Equal(t, "美团外卖", result.ZH)
        assert.Equal(t, "", result.TH)
        assert.Equal(t, "Meituan", result.EN)
    })
    ...
    t.Run("Snapshot JSON invalid", func(t *testing.T) {
        saleBill := &SaleBill{
            OrderSourceName: "invalid json", // JSON 格式错误
            OrderSource: &OrderSource{
                MultiLanguageName: &MultiLanguageName{
                    ZhName: "美团外卖",
                    ThName: "จีน",
...
#### 测试 SetOrderSourceNameSnapshot() 方法

```go
func TestSaleBill_SetOrderSourceNameSnapshot(t *testing.T) {
    // 测试场景1: 正常设置快照
    t.Run("Set snapshot successfully", func(t *testing.T) {
        ...
        err := saleBill.SetOrderSourceNameSnapshot(multiLangName)
        
        assert.NoError(t, err)
        assert.NotEmpty(t, saleBill.OrderSourceName)
        
        // 验证 JSON 格式
        var result dto.LocaleResponse
        err = json.Unmarshal([]byte(saleBill.OrderSourceName), &result)
        assert.NoError(t, err)
        assert.Equal(t, "美团外卖", result.ZH)
        assert.Equal(t, "จีน", result.TH)
        assert.Equal(t, "Meituan", result.EN)
    ...
    t.Run("MultiLanguageName is empty", func(t *testing.T) {
        ...
        assert.NoError(t, err)
        assert.Empty(t, saleBill.OrderSourceName)
    })
}
```
...
func TestOrder_Create_OrderSourceSnapshot(t *testing.T) {
    // 1. 创建外卖来源配置
    orderSource := createOrderSource("美团外卖", "จีน", "Meituan")
    
    // 2. 创建订单
    order := createOrder(orderSource.Uuid)
    ...
    // 验证 JSON 格式
    var snapshot dto.LocaleResponse
    err := json.Unmarshal([]byte(saleBill.OrderSourceName), &snapshot)
    assert.NoError(t, err)
    assert.Equal(t, "美团外卖", snapshot.ZH)
    
    // 4. 删除外卖来源配置
    deleteOrderSource(orderSource.Uuid)
    
    // 5. 查询订单，验证仍然显示快照数据
    orderDetail, _ := getOrderDetail(order.Uuid)
    assert.Equal(t, "美团外卖", orderDetail.OrderSourceName.ZH)
}
```
...
```go
func TestOrder_Query_OrderSourceSnapshot(t *testing.T) {
    // 1. 创建历史订单（快照字段为空）
    historicalOrder := createHistoricalOrder()
    
    // 2. 查询订单，验证降级逻辑正常
    orderDetail, _ := getOrderDetail(historicalOrder.Uuid)
    assert.NotEmpty(t, orderDetail.OrderSourceName.ZH)
}
```
...
2. **索引优化**:
   - `order_source_uuid` 已有索引，降级查询时使用
...
- [ ] 实现 `GetLocaleOrderSourceName()` 方法
- [ ] 实现 `SetOrderSourceNameSnapshot()` 方法
- [ ] 编写单元测试（覆盖率 100%）
- [ ] 修改下单逻辑，保存快照
- [ ] 修改查询逻辑，使用快照
