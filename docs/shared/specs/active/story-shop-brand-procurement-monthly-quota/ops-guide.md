...
| 特性 | 说明 | 运维关注点 |
|------|------|------------|
| **实时统计** | 不维护额外状态，直接查询订单统计 | 需关注查询性能 |
| **自动释放** | 驳回订单自动释放额度 | 无需手动干预 |
| **物品级控制** | 通过 `material_code` + `unit_code` 精确限制 | 配置颗粒度细 |
| **门店维度** | 每个门店独立统计 | 数据隔离性好 |
| **总部管控** | 配置存储在总部数据库 | 配置需同步到总部库 |
| **周期灵活** | 物品限额支持按天/月度 | 满足不同业务需求 |
...
| 项目 | 要求 | 说明 |
|------|------|------|
| MySQL 版本 | >= 8.0 | 需要支持 JSON 函数 |
| Go 版本 | >= 1.23 | Main 模块运行时 |
| Redis 版本 | >= 6.0 | 缓存支持（可选） |
| 磁盘空间 | >= 10GB | 预留日志和数据增长空间 |
...
□ 备份策略
  ├─ 配置表备份计划（ttpos_setting, purchase_quota_config）
  ├─ 订单表备份计划（ttpos_purchase_order）
  └─ 回滚方案准备
...
```
✔ 20260108164925_add_purchase_brand_limit_config
✔ 20260108164942_create_ttpos_purchase_quota_config_table
✔ 20260108165005_create_ttpos_purchase_quota_config_shop_table
```
