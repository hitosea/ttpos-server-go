# 开账接口支持 PaymentID 任务分解

> 本文档定义开账接口支持 PaymentID 的详细执行任务清单。
...
- [ ] 1.1 修改 Protobuf 定义
  ...
  - Prompt:
    ...
    Task: 修改 selling.proto 中的 OpenPosEntryDetail 消息定义，增加 payment_id 字段支持
    ...
    2. 新增 payment_id 字段（optional string，字段编号 3）
...
- [ ] 1.3 验证 Protobuf 生成结果
  ...
  - Verification:
    - 检查 `OpenPosEntryDetail` 结构体包含 `PaymentId *string` 字段
...
- [ ] 2.1 实现参数校验逻辑
  ...
  - Changes:
    ...
    - 如果同时为空，返回错误：`open_pos_entry_detail[{index}]: payment_id 和 mode_of_payment 不能同时为空`
  - Prompt:
    ```
