...
| 创建时间   | 2025-12-22                                                    |
| 版本       | v2.12.0                                                       |
| 关键字     | 商品管理, 删除限制, 属性设置, 加料设置, 套餐分组, 可选范围   |
...
1. **删除限制不完善**：商品或规格可以在有外卖订单未完结时被删除，可能导致订单数据异常
2. **属性/加料设置不灵活**：当前只能设置"必选"和"最大可选"，无法设置"最小可选"，不能表达"至少选1个，最多选3个"的业务需求
3. **套餐分组设置单一**：可选分组只能设置"可选数量"，无法设置可选范围
4. **总部数据编辑权限不清晰**：来源总部的数据编辑权限需要明确界定

### 1.2 目标

1. **增强删除保护**：防止删除处于未完结外卖订单中的商品或规格
2. **优化属性/加料设置**：支持可选范围（最小-最大）设置，替代原有的"必选+最大可选"模式
3. **优化套餐分组设置**：支持可选范围（最小-最大）设置
4. **明确编辑权限**：总部数据允许修改外卖渠道的价格
5. **数据兼容**：旧数据能够正确转换为新的可选范围模式
...
**作为**店长或运营人员  
**我想**在删除商品或规格时能得到明确提示  
**以便于**避免误删除正在外卖订单中的商品，保证订单数据完整性

**验收标准：**

- AC1：商品或规格处于未完结外卖订单中时，删除操作应被阻止
- AC2：删除失败时提示："商品或规格处于未完结外卖订单中，暂时无法删除"
- AC3：未完结外卖订单是指订单状态为：待接单、待配送、配送中等非最终状态
...
1. 查询 `ttpos_sale_order` 表，检查是否存在该商品/规格的未完结外卖订单
2. 未完结外卖订单的定义：
   - 订单类型为外卖订单
   - 订单状态不为：已完成、已取消、已退款等最终状态

#### 3.1.2 删除阻止

- 如果存在未完结外卖订单，阻止删除操作
- 返回错误提示："商品或规格处于未完结外卖订单中，暂时无法删除"
