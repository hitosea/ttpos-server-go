...
| 项目              | 内容                                                                                                                |
| ----------------- | ------------------------------------------------------------------------------------------------------------------- |
| **来源 Proposal** | [docs/team/proposals/2025-11/admin-role-permission.md](../../../../team/proposals/2025-11/admin-role-permission.md) |
| **创建日期**      | 2025-11-24                                                                                                          |
| **负责人**        | 曾振华                                                                                                              |
...
- [x] 0.4 使用 `constant.ShopAppRouteName`（"管理 APP"）作为路由名称获取新管理端权限
- [x] 0.5 权限数据经过筛选（根据商户类型、ERP 对接状态、渠道营收统计配置）
...
1. **WHEN** 管理员调用权限树接口 **THEN** 系统 **SHALL** 返回店铺的权限树（管理 APP、收银机、点餐助手三个权限组，不包含管理后台）
2. **WHEN** 管理员调用角色权限接口 **THEN** 系统 **SHALL** 返回该角色的权限 UUID 列表
3. **权限数据经过筛选**: 根据商户类型、ERP 对接状态、渠道营收统计配置等动态筛选
4. **不依赖员工**: 权限树接口不依赖员工权限，返回店铺的所有可用权限
5. **过滤管理后台**: 权限树不返回"管理后台"分组，只返回"管理 APP"、"收银机"、"点餐助手"三个分组
6. **商家后台隐藏管理 APP 权限**: 商家后台（PHP Admin）获取权限列表时应隐藏管理 APP 权限（uuid=2856266502144000），仅平台管理端可见
7. **国际化支持**: 权限名称根据请求头的 Accept-Language 返回对应语言，支持中文（zh）、英文（en）等 10 种语言
...
- [x] 0.5.4 权限数据经过筛选（根据商户类型、ERP 对接状态、渠道营收统计配置）
...
- [x] 0.5.8 权限名称支持国际化，根据请求头 Accept-Language 返回对应语言的权限名称（使用 i18n.Translate 实现）

**技术说明**:

- Service 层新增 `GetCompanyPermissionGroup` 方法（不依赖员工，只依赖店铺设置），接受 `includeRouteNames` 参数用于指定需要返回的权限组
- Service 层新增 `GetRolePermissions` 方法（获取角色的权限 UUID 列表）
- `/api/v1/shop/permission_tree` 接口在 `shop_staff.go` 中实现，返回不包含"管理后台"的权限树
- `/api/v1/shop/permission_group` 接口在 `shop_role.go` 中实现，用于角色权限配置，通过 `includeRouteNames` 参数指定返回"管理 APP"、"收银机"、"点餐助手"三个权限组

**实现细节**:

- API 层调用时传入三个权限组常量：`constant.ShopAppRouteName`（"管理 APP"）、`constant.CashierRouteName`（"收银机"）、`constant.AssistantRouteName`（"点餐助手"）
...
5. **IF** 散户商户 **THEN** 系统 **SHALL** 隐藏品牌采购、品牌收货、调拨单权限选项
6. **IF** 未对接 ERP **THEN** 系统 **SHALL** 隐藏进销存权限选项
7. **IF** 关闭渠道营收统计 **THEN** 系统 **SHALL** 隐藏首页渠道营收统计的"更多"选项

#### 具体要求

- [ ] 1.1 在角色管理页面添加"添加角色"按钮
- [ ] 1.2 创建角色添加表单，包含角色名称输入框（必填，1-50 字符）
- [ ] 1.3 添加功能权限配置区域，支持管理 APP、收银机、点餐助手三个模块
- [ ] 1.4 管理 APP 权限默认勾选所有权限
- [ ] 1.5 根据商户类型和配置动态显示/隐藏权限选项：
  - 总部商户隐藏品采收货权限
  - 散户商户隐藏品牌采购、品牌收货、调拨单权限
  - 未对接 ERP 隐藏进销存权限
...
- [x] 2.7 在获取角色详情时，返回公司管理 APP、收银机、点餐助手三个权限组的叶子节点数量之和（`total_leaf_count`）
...
1. **IF** 商家后台访问 **THEN** 系统 **SHALL** 隐藏管理 APP 权限选项（uuid=2856266502144000）
2. **IF** 总部商户 **THEN** 系统 **SHALL** 隐藏品采收货权限选项
3. **IF** 散户商户 **THEN** 系统 **SHALL** 隐藏品牌采购（uuid=2858468511744000）、品牌收货（uuid=2858548203520000）、调拨单（uuid=2858825027584000）权限选项
4. **IF** 未对接 ERP **THEN** 系统 **SHALL** 隐藏进销存权限选项
5. **IF** 关闭渠道营收统计 **THEN** 系统 **SHALL** 隐藏首页渠道营收统计的"更多"选项

#### 具体要求

- [x] 4.1 商家后台隐藏管理 APP 权限（在 `admin/app/common/model/shop/Access.php` 中过滤 uuid=2856266502144000）
- [x] 4.2 在权限配置界面，根据商户类型判断是否显示品采收货权限（总部）和品牌采购、品牌收货、调拨单权限（散户）
- [ ] 4.3 在权限配置界面，根据 ERP 对接状态判断是否显示进销存权限
- [ ] 4.4 在权限配置界面，根据渠道营收统计配置判断是否显示相关权限
- [x] 4.5 权限筛选逻辑与现有权限筛选逻辑保持一致（参考 `main/app/service/role_access.go`）
...
- [x] 所有文案使用多语言实现
- [x] **权限名称国际化**: 获取权限树接口需根据请求头 Accept-Language 返回对应语言的权限名称
  - 从请求头中获取 Accept-Language（如：zh、en、ja、ko 等）
  - 默认语言为英文（en）
  - 使用 `i18n.Translate()` 函数动态翻译权限名称
  - 翻译内容存储在 `main/i18n/languages/*.json` 文件中
  - 如果指定语言的翻译不存在，返回原始权限名称作为 fallback
  - 在 `GetCompanyPermissionGroup` 方法中递归翻译所有权限节点
...
2. **添加角色**: 管理员可以创建角色，配置功能权限，管理 APP 默认勾选所有权限
...
7. **国际化支持**: 获取权限树接口根据请求头 Accept-Language 返回对应语言的权限名称，未指定时默认返回中文
...
- 管理 APP 权限数据已初始化（`admin/database/migrations/20251124014502_init_management_app_access.php`）
- 权限筛选逻辑正常工作
...
- `admin/database/migrations/20251124014502_init_management_app_access.php` - 管理 APP 权限初始化
