...
5. **IF** 未对接 ERP **THEN** 系统 **SHALL** 隐藏进销存权限选项
...
3. **IF** 未对接 ERP **THEN** 系统 **SHALL** 隐藏进销存权限选项
4. **IF** 关闭渠道营收统计 **THEN** 系统 **SHALL** 隐藏首页渠道营收统计的"更多"选项

#### 具体要求

- [x] 4.1 商家后台隐藏管理 APP 权限（在 `admin/app/common/model/shop/Access.php` 中过滤 uuid=2856266502144000）
- [ ] 4.2 在权限配置界面，根据商户类型判断是否显示品采收货权限
- [ ] 4.3 在权限配置界面，根据 ERP 对接状态判断是否显示进销存权限
- [ ] 4.4 在权限配置界面，根据渠道营收统计配置判断是否显示相关权限
- [ ] 4.5 权限筛选逻辑与现有权限筛选逻辑保持一致（参考 `main/app/service/role_access.go`）
