...
在新管理端（Shop 商家管理端）新增支付管理功能模块，提供完整的支付方式管理 API 接口，支持商户通过 API 自主管理支付方式，包括添加、编辑、状态变更（启用/禁用）等操作。同时提供 LianlianPay 专项配置 API，支持配置白名单 IP、商户号、支付方式公钥、商户私钥、Token、站点 ID 等参数。
...
**作为** 商户管理员  
**我想** 在管理端自主管理支付方式，包括添加、编辑、启用/禁用，以及配置 LianlianPay 的相关参数  
...
- [ ] 2.1 API 请求字段：名称（必填）、代码（必填）、支付名称（必填）、来源（必填）、Logo 图片 UUID、二维码图片 UUID、手续费百分比、显示设置（收银机/助手/自助点餐机/会员充值）、状态、排序
...
### Requirement 5: LianlianPay 专项配置 API
...
2. **IF** 配置了敏感信息（私钥、Token） **THEN** 系统 **SHALL** 使用 RSA 加密传输到支付服务获取签名盐
3. **IF** 必填配置项未填写 **THEN** 系统 **SHALL** 返回错误并阻止保存
4. **WHEN** 配置保存成功 **THEN** 系统 **SHALL** 返回配置信息（敏感字段返回占位符 `***`）
5. **WHEN** 更新配置 **THEN** 系统 **SHALL** 通过 RSA 加密与支付服务通信，获取签名盐（sign_salt）并存储

#### 具体要求

- [x] 6.1 配置项包括：白名单 IP、商户号、支付方式公钥、商户私钥、Token、站点 ID、签名盐（sign_salt）
- [x] 6.2 敏感字段（商户私钥、Token）需通过 RSA 加密传输到支付服务（使用 RSA 加密算法）
- [x] 6.3 敏感字段在 API 响应中返回占位符 `***`（不返回明文）
- [x] 6.4 支持配置项的查询和更新（按公司 UUID 关联）
- [x] 6.5 配置项验证：IP 格式、密钥格式等（参数验证）
- [x] 6.6 配置保存后实时生效（通过 RSA 加密与支付服务通信，获取签名盐并存储）
...
- [x] 所有 API 需要身份验证
- [x] 敏感数据传输加密（LianlianPay 私钥、Token 通过 RSA 加密传输到支付服务）
- [x] SQL 注入防护（使用参数化查询）
- [x] CSRF 防护（Token 验证）
- [x] RSA 加密工具包（`main/pkg/encrypt/rsa.go`）用于安全通信
...
5. **LianlianPay 配置 API**: 能够正确配置 LianlianPay 参数，敏感信息加密存储
...
- 系统来源的支付方式（source=0）部分字段不可修改
- LianlianPay 配置参数敏感，必须通过 RSA 加密传输
- 支付方式状态变更需实时同步到收银端
