# ClosePosEntry PaymentID 支持 - 集成测试指南

> 本文档说明如何进行完整的集成测试，验证 ClosePosEntry 接口的 PaymentID 支持功能。
...
### 场景 1: 使用 payment_id 关账（成功）

**目的**: 验证提供 payment_id 时，系统能自动查询 mode_of_payment 并成功关账

**前置步骤**:
1. 创建一个 POS Opening Entry
2. 获取一个有效的 payment_id（通过 GetModeOfPaymentList 接口）

**测试请求**:
```json
{
  "pos_open_entry_name": "POS-OPEN-2025-001",
  "period_end_date": 1703980800,
...
- 关账记录创建成功
- 日志中显示 "关账详情: 通过 payment_id 查询到 mode_of_payment"

**验证方法**:
1. 检查响应状态码
2. 查看 ttpos-erp 服务日志
3. 在 ERPNext 中验证 POS Closing Entry 是否创建成功

---

### 场景 2: 使用 mode_of_payment 关账（向后兼容）
...
**预期结果**:
- 返回错误响应
- 错误信息包含: "payment_id 和 mode_of_payment 不能同时为空"
- 包含 detail 的索引
...
3. 调用 ClosePosEntry:
```bash
grpcurl -plaintext -d @ localhost:8080 selling.Selling/ClosePosEntry <<EOF
{
  "pos_open_entry_name": "POS-OPEN-2025-001",
  "period_end_date": 1703980800,
  "close_pos_entry_detail": [
...
### 成功场景
```
[INFO] 关账详情: 通过 payment_id 查询到 mode_of_payment
  index: 0
  payment_id: PID1234567890123456
  mode_of_payment: Cash
```
...
- [ ] 场景 1: payment_id 关账成功
- [ ] 场景 2: mode_of_payment 关账成功（向后兼容）
- [ ] 场景 3: 参数校验正确（都为空时失败）
