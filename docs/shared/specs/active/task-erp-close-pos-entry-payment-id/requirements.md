# 关账接口支持 PaymentID 需求文档

> 本文档定义关账接口支持 PaymentID 的详细需求和验收标准。

## 📋 基本信息

| 项目              | 内容                                                                                                         |
| ----------------- | ------------------------------------------------------------------------------------------------------------ |
| **来源 Proposal** | [docs/team/proposals/2025-12/close-pos-entry-payment-id-support.md](../../../../team/proposals/2025-12/close-pos-entry-payment-id-support.md) |
| **创建日期**      | 2025-12-24                                                                                                 |
| **负责人**        | -                                                                                                       |
| **目标 Sprint**   | Sprint -                                                                                                   |
...
| **审核人**   | rikugun             |
| **审核日期** | 2025-12-24             |
| **审核意见** | 技术任务，需求明确，可以进入设计阶段         |

---

## 📋 概述

在 `ClosePosEntry` 接口的 `ClosePosEntryDetail` 消息中增加 `payment_id` 可选参数，并调整 `mode_of_payment` 为可选字段。当调用方提供 `payment_id` 时，系统自动查询对应的 `mode_of_payment` 完成关账操作，简化调用流程并与其他接口保持设计一致。

**核心价值**：
- 简化调用流程，无需额外查询 `mode_of_payment`
- 与 `SavePosInvoice` 等接口保持一致的设计模式
...
1. **WHEN** 定义 `ClosePosEntryDetail` 消息 **THEN** 系统 **SHALL** 将 `mode_of_payment` 字段改为 `optional string` 类型
2. **WHEN** 定义 `ClosePosEntryDetail` 消息 **THEN** 系统 **SHALL** 新增 `payment_id` 字段（`optional string`，字段编号为 4）
3. **WHEN** 修改 Protobuf 定义后 **THEN** 系统 **SHALL** 通过 `gf gen pb` 重新生成 API 文件

#### 具体要求

- [x] 1.1 `ClosePosEntryDetail.mode_of_payment` 改为 `optional string` 类型
- [x] 1.2 新增 `ClosePosEntryDetail.payment_id` 字段（`optional string`）
- [x] 1.3 添加字段注释说明两个字段的关系和使用场景
- [x] 1.4 保持其他字段不变（`opening_amount`、`closing_amount`）
...
1. **WHEN** 调用 `ClosePosEntry` 接口 **AND** `payment_id` 和 `mode_of_payment` 同时为空 **THEN** 系统 **SHALL** 返回参数错误（错误信息：`payment_id 和 mode_of_payment 不能同时为空`）
2. **WHEN** 调用 `ClosePosEntry` 接口 **AND** 仅提供 `payment_id` **THEN** 系统 **SHALL** 接受该请求并进行后续处理
3. **WHEN** 调用 `ClosePosEntry` 接口 **AND** 仅提供 `mode_of_payment` **THEN** 系统 **SHALL** 接受该请求并进行后续处理
4. **WHEN** 调用 `ClosePosEntry` 接口 **AND** 同时提供 `payment_id` 和 `mode_of_payment` **THEN** 系统 **SHALL** 优先使用 `payment_id` 进行处理

#### 具体要求

- [x] 2.1 在 Logic 层添加参数校验逻辑
- [x] 2.2 校验逻辑在处理 `ClosePosEntryDetail` 列表时对每个 detail 进行检查
- [x] 2.3 返回的错误信息应明确指出参数问题
- [x] 2.4 使用 GoFrame 的 `gerror` 包进行错误处理

---

### Requirement 3: 自动查询 Mode of Payment

**用户故事**: 作为系统，我需要在提供 `payment_id` 时自动查询对应的 `mode_of_payment`，以便于完成关账操作

#### 验收标准

1. **WHEN** `payment_id` 不为空 **THEN** 系统 **SHALL** 调用 `GetModeOfPayment` 服务通过 `payment_id` 查询对应的 `mode_of_payment`
2. **IF** `payment_id` 查询成功 **THEN** 系统 **SHALL** 使用查询得到的 `mode_of_payment` 进行后续关账处理
...
- [x] 3.1 在 Logic 层的 `ClosePosEntry` 方法中添加查询逻辑
- [x] 3.2 调用 `service.Selling().GetModeOfPayment(ctx, &selling.GetModeOfPaymentReq{PaymentId: detail.PaymentId})`
- [x] 3.3 处理查询失败的情况，使用 `gerror.Wrapf` 包装错误
- [x] 3.4 查询成功后提取 `resp.ModeOfPayment.Name` 用于后续处理
- [x] 3.5 保持原有使用 `mode_of_payment` 的逻辑不变（向后兼容）
...
- [x] 4.1 保持原有 `mode_of_payment` 字段可用
- [x] 4.2 不修改现有使用 `mode_of_payment` 的代码逻辑
- [x] 4.3 只在 `payment_id` 不为空时执行新增的查询逻辑
- [x] 4.4 确保 Protobuf 字段变更为 optional 后不破坏现有序列化/反序列化
...
- [x] 响应格式通过 `erp.ResponseInfo` 包装
- [x] 错误信息使用中文，便于运维和调试
- [x] 字段命名使用 snake_case
- [x] 参考: `ttpos-bmp/.cursor/rules/proto-rules.mdc` - Protobuf 规范
...
- [x] 查询 `GetModeOfPayment` 响应时间 < 100ms
- [x] 整体接口响应时间不因新增查询而显著增加
- [x] 考虑缓存优化（如 `GetModeOfPayment` 内部已有缓存）
...
- [x] 覆盖以下测试场景：
  ...
  - 同时提供两个参数时的优先级处理
- [x] 单元测试覆盖参数校验逻辑
- [x] 集成测试覆盖与 `GetModeOfPayment` 的交互

### 安全要求

- [x] gRPC 接口需要身份验证（已有机制）
- [x] 参数校验防止无效输入
- [x] 错误信息不泄露敏感数据

### 可靠性要求

- [x] 网络异常时优雅降级（查询失败返回明确错误）
- [x] 错误日志记录（使用 GoFrame Logger）
- [x] 错误信息包含上下文（如 `payment_id` 值）
...
1. **Protobuf 定义**: `ClosePosEntryDetail` 包含 `optional string payment_id` 字段，`mode_of_payment` 改为 optional
...
- `docs/team/proposals/2025-12/close-pos-entry-payment-id-support.md` - 本功能的需求提案
- `docs/team/proposals/2025-12/payment-id-update-logic.md` - 支付方式 PaymentID 更新逻辑提案
