...
快捷支付功能旨在简化POS收银流程，通过一键支付和快捷键支持，将平均结账时间从30秒缩短至3秒，提升收银效率30%，改善高峰期顾客等待体验。

本功能主要涉及订单支付API优化、缓存设计优化、以及管理后台配置功能。
...
### Requirement 1: 快捷支付API

**用户故事**: 作为POS客户端，我想调用快捷支付API一键完成支付，以便于简化支付流程
...
- [ ] 2.2 管理后台增加配置页面（Vue）
- [ ] 2.3 创建配置更新API
- [ ] 2.4 配置数据缓存到Redis（Key: `ttpos:company:config:{company_uuid}`）
...
- [x] `ttpos_company` 表增加 `default_payment_method` 字段（int, default 1表示现金）
...
- [x] 快捷支付API本地响应时间 < 200ms（P0）
- [x] 商户配置数据缓存到Redis，缓存时间30分钟
- [x] 订单查询使用uuid索引优化
- [x] 并发场景使用UUID锁防止重复支付
...
- [x] 集成测试覆盖完整支付流程
- [x] 并发测试：10个并发请求同一订单，只能成功1次
- [x] 参考: `.cursor/rules/go-main.mdc` - 测试规范

### 安全要求

- [x] 快捷支付API需要身份验证（JWT Token）
...
1. **快捷支付流程**: 调用API → 订单状态更新 → 支付记录创建 → 事件发布 → 小票打印
...
1. **技术文档**: design.md 包含完整的数据库设计、API设计、缓存设计
...
- Service 依赖 IOrderService 和 IPaymentService 接口（不直接依赖Repository）
