...
- **显示位置**：在分组商品配置中显示（固定分组和可选分组都支持）
- **固定分组**：可以设置必选和默认选中，但通常固定分组的所有商品都是必选的
- **可选分组**：显示必选和默认选中复选框，用于控制商品选择行为
...
- **分组类型切换**：
  - 从"可选"切换到"固定"时，保持现有的 `is_required` 和 `is_default` 值
  - 从"固定"切换到"可选"时，保持现有的 `is_required` 和 `is_default` 值
...
- **后端验证**：
  ...
  - 可选分组时，验证必选数量和默认数量不能大于可选数量

- **前端验证**：
  - 实时验证必选数量和默认数量（仅可选分组）
  - 分组类型切换时，保持现有的必选和默认选中值
