...
type WarehouseOutForm struct {
	ID           int     `gorm:"primaryKey;column:id;comment:出库单唯一标识符"`
...
// BeforeCreate 创建前钩子
func (m *WarehouseOutForm) BeforeCreate(tx *gorm.DB) error {
...
// BeforeUpdate 更新前钩子
func (m *WarehouseOutForm) BeforeUpdate(tx *gorm.DB) error {
	m.UpdateTime = time.Now().Unix()
	return nil
}
