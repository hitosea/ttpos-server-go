...
func (s *StatisticsProductService) convertProduct(offset int, limit int) error {
	...
	err := s.db.Raw(`
        SELECT
			...
			IF( rp.tax_calc_type = 2, 
				rp.total_pay_price / rp.total_num - if(rp.refund_money > 0, rp.refund_money - (rp.product_service_fee / rp.total_num), 0), 
				rp.total_pay_price / rp.total_num - (rp.product_consumption_tax / rp.total_num) - if(rp.refund_money > 0, (rp.refund_money - (rp.product_service_fee / rp.total_num) - (rp.product_service_consumption_tax / rp.total_num)), 0)
			) as product_price,
