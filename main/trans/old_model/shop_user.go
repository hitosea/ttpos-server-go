...
type ShopUser struct {
	...
	PasswordChange   int    `gorm:"default:0;comment:修改密码次数"`
	RealName         string `gorm:"default:'';comment:姓名"`
	IsSuper          uint8  `gorm:"default:0;comment:是否为超级管理员0不是,1是"`
	ShopSupplierID   int    `gorm:"default:0;comment:总店id"`
	IsDelete         uint8  `gorm:"default:0;comment:0=显示1=伪删除"`
	UserType         uint8  `gorm:"default:0;comment:账号类型0总台1门店"`
	IsStatus         uint8  `gorm:"default:0;comment:是否禁用1禁用，0未禁用"`
	AppID            uint   `gorm:"default:0;comment:程序id"`
	BindKey          string `gorm:"default:'';comment:绑定的设备key"`
	CashierOnline    uint8  `gorm:"default:0;comment:收银员当班 0-不在线 1-在线"`
	CashierLoginTime int    `gorm:"default:0;comment:收银员当班登录时间"`
