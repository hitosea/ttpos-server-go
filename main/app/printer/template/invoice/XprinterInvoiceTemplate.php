 ...
 * 心烨 发票模版
 */
class XprinterInvoiceTemplate extends BaseTemplate
{
    /**
     * 生成模版
     */
    public function create($order, $printType, $isCashierPrinter)
    {
        ...
        */
        if ($template == 2) {
            ...
            $order->_template = 3;
            $order->_title = __("发票");
            return (new XprinterBillTemplate($this->setting, $this->allSourceProductList, $this->isSunmi))->create($order, $printType);
        }
        ...
        */
        $width = 48;
        $leftWidth = 28;
        $isXinyeWifi = $printType == PrinterTypeEnum::XPRINTER_WIFI || $printType == PrinterTypeEnum::CODESOFT_LAN;
        ...
        if ($order->invoiceInfo && ($order->invoiceInfo->company_name || $order->invoiceInfo->company_addr || $order->invoiceInfo->company_tax_number || $order->invoiceInfo->company_phone)) {
            ...
            if ($order->invoiceInfo->company_addr) {
                ...
                $printer->setLineSpacing($isXinyeWifi ? 48 : 40);
                $printer->lineFeed(2);
            }
