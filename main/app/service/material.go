...
func (s *materialSrv) GetMaterialList(ctx context.Context, req req.MaterialListReq) (material_resp.MaterialListWithPaginationResp, error) {
	...
	for _, material := range materials {
		...
		if companySetting.IsSubShop() && material.HeadquarterUuid > 0 {
			respMaterial.IsSafetyStockEditable = true // 安全库存始终允许子店编辑（使用 override 追踪）
			respMaterial.IsNegativeStockEditable = !isUnifiedFromMap(hqControlMap, constant.HqFieldNegativeStock)
		} else {
			respMaterial.IsSafetyStockEditable = true
			respMaterial.IsNegativeStockEditable = true
		}
...
func (s *materialSrv) GetMaterialDetail(ctx context.Context, req req.MaterialDetailReq) (material_resp.MaterialDetailResp, error) {
	...
	return material_resp.MaterialDetailResp{
		...
		IsNegativeStockEditable: func() bool {
			companySetting := ctx.GetCompanySetting()
			if companySetting.IsSubShop() && material.IsHeadquarter() {
				hqDb := s.dbm.GetDB(companySetting.HeadquarterUuid)
				hqControlMap := repository.GetHqControlMap(hqDb, companySetting.HeadquarterUuid)
				return !isUnifiedFromMap(hqControlMap, constant.HqFieldNegativeStock)
			}
			return true
		}(),
	}, nil
...
func (s *materialSrv) AddMaterial(ctx context.Context, req req.MaterialAddReq) error {
	...
	// HQ 添加物品 → 自动推送到子店
	companySetting := ctx.GetCompanySetting()
	if companySetting.IsHeadquarter() && materialUuid > 0 {
		hqPushSrv := NewHqPushSrv(s.dbm)
		utils.Go(func() {
			hqPushSrv.OnHqMaterialChanged(ctx, materialUuid)
		})
	}
...
func addMaterial(ctx context.Context, tx *gorm.DB, settingSrv setting.ISrv, request req.MaterialAddReq) (*model.Material, *req.MaterialAddErpReq, error) {
	...
	material := model.Material{
		...
		DefaultSalesUnitUuid:  unitMap[request.DefaultSalesUnitUuid],
	...
	if material.DefaultSalesUnitUuid != 0 {
		...
		if err != nil {
			return nil, nil, errors.WithMessage(err, "获取默认销售单位失败")
		}
		if defaultSalesUnit.Unit != nil {
			defaultSalesUom = defaultSalesUnit.Unit.ErpnextUom
		}
	}
	...
	materialAddErpReq := &req.MaterialAddErpReq{
		...
		DefaultSalesUnit:   defaultSalesUom,
		AllowNegativeStock: request.AllowNegativeStock,
	}
...
func (s *materialSrv) EditMaterial(ctx context.Context, request req.MaterialEditReq) error {
	...
	if err := repository.CommonRepo.Transaction(db, func(tx *gorm.DB) error {
		...
		material := model.Material{
			...
			DefaultSalesUnitUuid: func() uint64 {
				// 如果选择了已经存在的单位，则使用已存在的单位
				if _, ok := notBaseUnitList[request.DefaultSalesUnitUuid]; ok {
					return request.DefaultSalesUnitUuid
				}
				// 如果选择了新的单位，则创建新的单位
				return unitMap[request.DefaultSalesUnitUuid]
			}(),
			OriginCountryCode: request.OriginCountryCode,
		}
...
func (s *materialSrv) UpdateMaterialStatusBatch(ctx context.Context, request req.MaterialStatusReq) error {
	...
	if companySetting.IsHeadquarter() {
		...
		for _, uuid := range materialUuids {
			mu := uuid
			utils.Go(func() {
				hqPushSrv.OnHqMaterialChanged(ctx, mu)
			})
		}
	}
...
func (s *materialSrv) SyncMaterial(ctx context.Context, syncHeadquarterData bool) error {
		...
		for _, itemInfo := range materialList.ItemList {
			...
			if existingMaterial.Uuid != 0 { // 如果物品已存在
				...
				material, err := s.AddMaterialByEprItem(copyCtx, req.MaterialAddErpReq{
					...
					StockUom:           itemInfo.StockUom,
					PurchaseUom:        itemInfo.PurchaseUom,
					DefaultSalesUnit: func() string {
						if itemInfo.SalesUom != nil && *itemInfo.SalesUom != "" {
							return *itemInfo.SalesUom
						}
						return ""
					}(),
					NotForSale:           itemInfo.NotForSale,
	...
	if companySetting.IsSubShop() && syncHeadquarterData {
		...
		err = s.SyncHeadquarterMaterials(headquarterDb, ctx.GetDB(), companySetting.HeadquarterUuid)
	}
...
// SyncHeadquarterMaterials 同步总部物品到子店（delete+recreate 模式）
func (s *materialSrv) SyncHeadquarterMaterials(hqDb *gorm.DB, subDb *gorm.DB, hqUuid uint64) error {
	commonRepo := repository.NewCommonRepo()
	materialRepo := repository.NewMaterialRepo(hqDb)
	...
	return subDb.Transaction(func(tx *gorm.DB) error {
		commonRepo := repository.NewCommonRepo()
		materialRepo := repository.NewMaterialRepo(tx)
		materialUnitRepo := repository.NewMaterialUnitRepo(tx)
		...
		subShopMaterialNegativeStockMap := make(map[uint64]int)
		...
		for _, subShopMaterial := range subShopMaterialList {
			...
			subShopMaterialNegativeStockMap[subShopMaterial.Uuid] = subShopMaterial.AllowNegativeStock
