...
func (s *warehouseSrv) GetWarehouseList(ctx context.Context, req req.WarehouseListReq) (resp.WarehouseListResp, error) {
	...
	// 排序
	opts = append(opts, warehouseRepo.OrderByCreateTime(true), warehouseRepo.WhereCompanyAbbr(ctx.GetCompanySetting().ErpnextCompanyAbbr))
	// 分页查询
...
func (s *warehouseSrv) CreateWarehouse(ctx context.Context, addReq req.CreateWarehouseReq) error {
	...
	warehouse.CompanyAbbr = companySetting.ErpnextCompanyAbbr
...
func (s *warehouseSrv) UpdateWarehouse(ctx context.Context, updateReq req.UpdateWarehouseReq) error {
	...
	if !isEditable(ctx, existingWarehouse.CompanyAbbr) {
...
func (s *warehouseSrv) buildWarehouseResp(ctx context.Context, warehouse model.Warehouse) resp.WarehouseResp {
	...
	return resp.WarehouseResp{
		...
		Editable:  isEditable(ctx, warehouse.CompanyAbbr),
	}
}
...
func (s *warehouseSrv) SetDefaultWarehouse(ctx context.Context, req req.SetDefaultWarehouseReq) error {
	...
	if !isEditable(ctx, exists.CompanyAbbr) {
		...
		for _, erpWarehouse := range warehouseList {
			...
			if warehouse.Uuid > 0 { // 如果存在，则更新
				...
				db.Model(&model.Warehouse{}).Where("uuid = ?", warehouse.Uuid).Updates(map[string]any{
					...
					"type":         warehouseType,
					"status":       status,
					"company_abbr": erpWarehouse.CompanyAbbr,
				})
			} else { // 新增
				var isDefault int
				...
				tx.Model(&model.Warehouse{}).Create(&model.Warehouse{
					...
					CompanyAbbr:           erpWarehouse.CompanyAbbr,
				})
			}
		}
		return nil
