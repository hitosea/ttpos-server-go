...
type TakeoutOrderPromo struct {
	...
	// 金额信息（单位：分）
	PromoAmount      float64 `gorm:"column:promo_amount" json:"promo_amount"`               // 促销金额
	MexFundedRatio   int32   `gorm:"column:mex_funded_ratio" json:"mex_funded_ratio"`       // 商户承担比例（百分比）
	MexFundedAmount  float64 `gorm:"column:mex_funded_amount" json:"mex_funded_amount"`     // 商户承担金额
	TargetedPrice    float64 `gorm:"column:targeted_price" json:"targeted_price"`           // 目标价格（订单小计）
	PromoAmountInMin float64 `gorm:"column:promo_amount_in_min" json:"promo_amount_in_min"` // 促销金额（最小单位）
}
