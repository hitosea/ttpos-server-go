...
**必须符合 Conventional Commits 规范（会被 `.husky/commit-msg` hook 验证）：**

格式: `{type}({scope}): {description}` 或 `{type}: {description}`

- `{type}`: 必须是上述类型之一
- `{scope}`: 可选，受影响的包/应用（如 pos、api、ui、model）
- `{description}`: 1-72 字符的简短描述

正则验证: `^(feat|fix|build|ci|docs|style|refactor|test|chore|perf)(\(.+\))?: .{1,72}$`

## 生成策略

1. 分析暂存文件的路径，识别受影响的包/应用作为 scope
2. 根据代码变更类型确定 type
...
当前分支必须符合 git flow 约定：
- `feature/*`, `hotfix/*`, `release/*`, `develop`, `main`

（由 hook 自动验证，merge 和 revert commits 除外）
