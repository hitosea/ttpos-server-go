---
name: unit-test
description: Write unit tests for Go packages (Main or BMP). Trigger when user asks to create, add, or write unit tests, or mentions testing a function, method, or package. These are package-level tests — NOT API integration tests.
...
2. **Classify the test category**:
   ...
   - **Service-dependent**: Requires Redis, MySQL, gRPC, or other external services
...
- Repository/DAO functions → Internal, MySQL required
- Cache/Lock functions → Internal, Redis required
- Service layer functions → External, may need mocking
...
- ❌ Hardcoded IP addresses (`192.168.x.x`, `156.255.x.x`)
...
```bash
cd main && go test -v -run "TestFunctionName" ./path/to/package/...
```

**Service-dependent tests (with build tag):**
```bash
cd main && go test -v -tags=integration -run "TestFunctionName" ./path/to/package/...
```
